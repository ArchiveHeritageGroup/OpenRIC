{{-- User Dashboard Display -- adapted from Heratio ahg-landing-page::my-dashboard --}}
            ...
            <div class="col-md-{{ $colSpan }}">
              <div class="{{ implode(' ', $blockClasses) }}"
                ...
                @if (view()->exists('openric-landing-page::blocks.' . $templateName))
                  @include('openric-landing-page::blocks.' . $templateName, ['block' => $block, 'config' => $config, 'data' => $data])
                @endif
              </div>
            </div>
          @endforeach
