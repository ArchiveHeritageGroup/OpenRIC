{{-- Block: Repository Spotlight -- adapted from Heratio ahg-landing-page --}}
@php
$repo = $data ?? null;
  ...
  <div class="repository-spotlight">
    <div class="row align-items-center">
      ...
      <div class="{{ $showLogo ? 'col-md-9' : 'col-12' }}">
        <h3 class="mb-2">
          <a href="{{ url('/repository/' . ($repo['slug'] ?? '')) }}"
          ...
          <div class="mt-3">
            ...
            <ul class="list-unstyled small">
              @foreach ($repo['sample_holdings'] as $holding)
                @php $holdingObj = is_object($holding) ? $holding : (object) $holding; @endphp
                <li>
                  <a href="{{ url('/record-resource/' . ($holdingObj->slug ?? '')) }}"
                     class="text-decoration-none">
                    {{ e($holdingObj->title ?? $holdingObj->slug ?? '') }}
                  </a>
                </li>
              @endforeach
            </ul>
