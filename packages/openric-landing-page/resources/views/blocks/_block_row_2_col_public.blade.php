{{-- Block: 2 Column Row Public Display -- adapted from Heratio ahg-landing-page --}}
@php
...
$col1Blocks = array_filter(is_array($childBlocks) ? $childBlocks : [], fn($b) => (is_object($b) ? ($b->column_slot ?? '') : ($b['column_slot'] ?? '')) === 'col1');
$col2Blocks = array_filter(is_array($childBlocks) ? $childBlocks : [], fn($b) => (is_object($b) ? ($b->column_slot ?? '') : ($b['column_slot'] ?? '')) === 'col2');
  ...
  <div class="row g-4">
    <div class="{{ $stackMobile ? 'col-12' : '' }} col-md-{{ $col1Md }}">
      @foreach ($col1Blocks as $childBlock)
        ...
        @endphp
        @if (view()->exists('openric-landing-page::blocks.' . $templateName))
          @include('openric-landing-page::blocks.' . $templateName, ['config' => $childConfig, 'data' => $childData, 'block' => $childBlock])
        @endif
      @endforeach
    </div>
