{{-- Block: GLAM Browser -- adapted from Heratio ahg-landing-page --}}
@php
$blockId = 'glam-browser-' . ($block->id ?? uniqid());
$browseUrl = url('/browse');
...
<div id="{{ $blockId }}" class="glam-browser-block"
  ...
  <div class="glam-browser-error" style="display: none;">
    <div class="alert alert-warning">
      ...
      <a href="{{ $browseUrl }}" class="alert-link">
  ...
  fetch(fetchUrl)
    ...
    .then(function(html) {
      ...
      content.querySelectorAll('a[href]').forEach(function(link) {
        ...
        if (href.includes('/record-resource/') || href.includes('/slug/')) return;
