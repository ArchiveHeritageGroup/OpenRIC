{{-- Partial: nested-block display within columns (editor view) -- adapted from Heratio --}}
@php
...
<div class="nested-block card mb-2" data-block-id="{{ $block->id }}">
  ...
  <div class="card-body py-2 px-2 bg-light small">
    @switch($block->machine_name)
      @case('text_content')
        <i class="bi bi-file-text"></i> {{ e(\Illuminate\Support\Str::limit(strip_tags($config['content'] ?? ''), 40)) }}...
        @break
      @case('search_box')
      ...
      @case('quick_links')
        ...
        @break
      @case('holdings_list')
        <i class="bi bi-book"></i> Holdings
        @break
      @case('image_carousel')
        <i class="bi bi-images"></i> Carousel
        @break
      @case('browse_panels')
        <i class="bi bi-folder"></i> Browse Panels
        @break
      @default
        {{ $block->type_label }}
    @endswitch
