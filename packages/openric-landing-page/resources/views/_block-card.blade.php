{{-- Partial: block-card for the editor canvas -- adapted from Heratio ahg-landing-page::_block-card --}}
@php
      ...
      <div class="row g-2">
        @for ($i = 1; $i <= $numCols; $i++)
          ...
          $colBlocks = collect(is_array($childBlocks) ? $childBlocks : (method_exists($childBlocks, 'toArray') ? $childBlocks->toArray() : []))
              ->filter(fn($b) => (is_object($b) ? ($b->column_slot ?? '') : ($b['column_slot'] ?? '')) === $colSlot);
          @endphp
          <div class="col">
            <div class="column-drop-zone border border-2 border-dashed rounded p-2 text-center"
              ...
              @else
                @foreach ($colBlocks as $childBlock)
                  @php $childObj = is_object($childBlock) ? $childBlock : (object) $childBlock; @endphp
                  <div class="nested-block card mb-1" data-block-id="{{ $childObj->id }}">
                    <div class="card-body py-1 px-2 small d-flex align-items-center">
                      ...
                      <span class="flex-grow-1 text-truncate">{{ $childObj->title ?? $childObj->type_label ?? 'Block' }}</span>
                      <button type="button" class="btn btn-link btn-sm p-0 px-1 btn-edit-nested text-primary"
                              data-block-id="{{ $childObj->id }}" title="Edit"><i class="bi bi-pencil"></i></button>
                      <button type="button" class="btn btn-link btn-sm p-0 btn-delete-nested text-danger"
                              data-block-id="{{ $childObj->id }}" title="Delete"><i class="bi bi-trash"></i></button>
                    </div>
                  </div>
                @endforeach
              @endif
    ...
    @else
      ...
      @switch($block->machine_name)
        ...
        @case('quick_links')
          ...
          @break
        @case('featured_items')
          <i class="bi bi-star"></i> {{ e($config['title'] ?? 'Featured Items') }}
          @break
        @case('header_section')
        ...
        @case('footer_section')
          ...
          @break
        @case('copyright_bar')
          <i class="bi bi-c-circle"></i> Copyright Bar
          @break
        @case('repository_spotlight')
          <i class="bi bi-building"></i> Repository Spotlight
          @break
        @case('map_block')
          <i class="bi bi-geo-alt"></i> Map
          @break
        @case('glam_browser')
          <i class="bi bi-grid"></i> GLAM Browser
          @break
        @case('divider')
