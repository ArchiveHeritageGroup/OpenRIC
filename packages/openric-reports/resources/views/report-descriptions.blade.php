    ...
    @include('reports::_filters', [
      ...
      'extraFilters' => '
        <div class="mb-3">
          ...
          <select name="repositoryId" class="form-select form-select-sm">
            <option value="">All repositories</option>' .
            ($repositories ?? collect())->map(fn($r) => '<option value="' . $r->id . '"' . (($params['repositoryId'] ?? '') == $r->id ? ' selected' : '') . '>' . e($r->name ?? '') . '</option>')->implode('') .
          '</select>
        </div>
        <div class="mb-3">
          ...
          <select name="level" class="form-select form-select-sm">
            ...
            ($levels ?? collect())->map(fn($l) => '<option value="' . $l->id . '"' . (($params['level'] ?? '') == $l->id ? ' selected' : '') . '>' . e($l->name) . '</option>')->implode('') .
          '</select>
        </div>
        <div class="mb-3">
          ...
          <select name="publicationStatus" class="form-select form-select-sm">
            ...
            <option value="draft"' . (($params['publicationStatus'] ?? '') === 'draft' ? ' selected' : '') . '>Draft</option>
            <option value="published"' . (($params['publicationStatus'] ?? '') === 'published' ? ' selected' : '') . '>Published</option>
          </select>
        </div>',
    ])
  </div>
