...
<div class="row">
  ...
  <div class="col-md-9">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <div>
        <a href="{{ route('reports.select') }}" class="btn btn-sm btn-outline-secondary"><i class="fas fa-arrow-left me-1"></i>Back to Reports</a>
        @if(!empty($results) && (is_countable($results) ? count($results) : $results->count()) > 0)
          <a href="{{ request()->fullUrlWithQuery(['export' => 'csv']) }}" class="btn btn-sm btn-outline-success ms-1"><i class="fas fa-file-csv me-1"></i>CSV</a>
        @endif
      </div>
    </div>
    ...
    {{-- Filter --}}
    <div class="card mb-3">
      <div class="card-header bg-primary text-white"><i class="fas fa-filter me-2"></i>Filter Options</div>
      <div class="card-body">
      ...
      <div class="card-body p-0">
        <div class="table-responsive">
          <table class="table table-bordered table-sm table-striped mb-0">
            <thead>
              <tr>
                @foreach($columns ?? ['ID','Identifier','Name','Entity Type','Created','Updated'] as $col)
                  <th>{{ $col }}</th>
                @endforeach
              </tr>
            </thead>
            <tbody>
              ...
              <tr>
                @foreach((array)$row as $val)
                  <td>{{ Str::limit((string)($val ?? ''), 80) ?: '-' }}</td>
                @endforeach
              </tr>
              @empty
              <tr><td colspan="{{ count($columns ?? ['ID','Identifier','Name','Entity Type','Created','Updated']) }}" class="text-center text-muted py-3">No results found</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
