...
<div class="row">
  <div class="col-md-3">
    ...
    <div class="card mb-3">
      <div class="card-header bg-primary text-white"><i class="fas fa-filter me-2"></i>Filters</div>
      <div class="card-body">
        <form method="get" action="{{ route('reports.activity') }}">
          ...
          <div class="mb-3">
            ...
            <select name="userAction" class="form-select form-select-sm">
              ...
              @foreach(['create', 'update', 'delete', 'search', 'login', 'logout'] as $a)
          ...
          <button type="submit" class="btn btn-outline-primary btn-sm w-100"><i class="fas fa-search me-1"></i>Filter</button>
        </form>
      </div>
    </div>
  </div>
  <div class="col-md-9">
    ...
    @if(!$auditTable)
      <div class="alert alert-warning">No audit log table found. Ensure the audit_log table exists.</div>
    @else
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-sm">
          <thead>
            <tr><th>Date</th><th>User</th><th>Action</th><th>Entity Type</th><th>Entity</th></tr>
          </thead>
          <tbody>
            @forelse($results as $row)
              <tr>
                ...
                <td>{{ $row->username ?? '' }}</td>
                <td>
                  @php $badge = match($row->action ?? '') { 'create' => 'bg-success', 'update' => 'bg-primary', 'delete' => 'bg-danger', 'search' => 'bg-info', default => 'bg-secondary' }; @endphp
                  <span class="badge {{ $badge }}">{{ $row->action ?? '' }}</span>
                </td>
                <td>{{ $row->entity_type ?? '' }}</td>
                <td>{{ $row->entity_title ?? $row->entity_id ?? '' }}</td>
              </tr>
            @empty
