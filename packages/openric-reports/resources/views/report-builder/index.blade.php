  ...
  <div class="col-md-9">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <div>
        ...
        <a href="{{ route('reports.builder.templates') }}" class="btn btn-outline-secondary btn-sm ms-1"><i class="fas fa-copy me-1"></i>From Template</a>
        <a href="{{ route('reports.builder.archive') }}" class="btn btn-outline-secondary btn-sm ms-1"><i class="fas fa-archive me-1"></i>Archive</a>
      </div>
    </div>
        ...
        <table class="table table-hover mb-0">
          ...
          <tbody>
            ...
            <tr class="report-row">
              ...
              <td>
                @php $statusClass = ['draft'=>'bg-secondary','active'=>'bg-info','published'=>'bg-success','archived'=>'bg-dark'][$report->status ?? 'draft'] ?? 'bg-secondary'; @endphp
                <span class="badge {{ $statusClass }}">{{ ucfirst($report->status ?? 'draft') }}</span>
              </td>
              <td>
              ...
              <td>
                <div class="btn-group btn-group-sm">
                  <a href="{{ route('reports.builder.preview', $report->id) }}" class="btn btn-outline-primary" title="View"><i class="fas fa-eye"></i></a>
                  <a href="{{ route('reports.builder.edit', $report->id) }}" class="btn btn-outline-secondary" title="Edit"><i class="fas fa-pencil-alt"></i></a>
                  <a href="{{ route('reports.builder.clone', $report->id) }}" class="btn btn-outline-info" title="Clone"><i class="fas fa-copy"></i></a>
                  <a href="{{ route('reports.builder.export', ['id' => $report->id, 'format' => 'csv']) }}" class="btn btn-outline-success" title="Export CSV"><i class="fas fa-download"></i></a>
                </div>
              </td>
            </tr>
            @endforeach
