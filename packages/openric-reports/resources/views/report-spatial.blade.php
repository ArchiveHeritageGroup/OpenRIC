    ...
    <div class="row">
      ...
      <div class="col-md-8">
        <form method="POST" action="{{ route('reports.spatial') }}">
          ...
          {{-- Coordinate Source --}}
          <div class="card mb-3">
            <div class="card-header bg-primary text-white"><i class="fas fa-crosshairs me-2"></i>Coordinate Source</div>
            <div class="card-body">
              <div class="mb-3">
                ...
                <select name="coordinateSource" class="form-select">
                  <option value="property" selected>Record latitude/longitude fields</option>
                </select>
                <div class="form-text">Coordinates are stored on the record_descriptions table.</div>
              </div>
            </div>
          </div>
          ...
          {{-- Record Filters --}}
          <div class="card mb-3">
            <div class="card-header bg-primary text-white"><i class="fas fa-filter me-2"></i>Record Filters</div>
            <div class="card-body">
              <div class="mb-3">
                <label class="form-label">Place <span class="badge bg-secondary ms-1">Optional</span></label>
              ...
              <div class="mb-3">
                ...
                <div class="form-text">Comma-separated list of subject terms to filter by (uses ILIKE matching).</div>
              </div>
          ...
          <div class="d-flex gap-2 mb-4">
            <button type="submit" class="btn btn-outline-primary" onclick="document.getElementById('exportField').value=''"><i class="fas fa-eye me-1"></i>Preview</button>
            <button type="submit" class="btn btn-outline-success" onclick="document.getElementById('exportField').value='csv'"><i class="fas fa-file-csv me-1"></i>Export CSV</button>
            <button type="submit" class="btn btn-outline-info" onclick="document.getElementById('exportField').value='json'"><i class="fas fa-file-code me-1"></i>Export GeoJSON</button>
          </div>
        </form>
        ...
        <div class="card mb-3">
          <div class="card-header bg-primary text-white"><i class="fas fa-table me-2"></i>Preview ({{ $preview->count() }} of {{ number_format($totalCount) }} records)</div>
          <div class="table-responsive">
            <table class="table table-bordered table-sm table-striped mb-0">
              <thead><tr><th>ID</th><th>Identifier</th><th>Title</th><th>Latitude</th><th>Longitude</th><th>Level</th><th>Repository</th></tr></thead>
              <tbody>
      ...
      <div class="col-md-4">
        <div class="card mb-3">
          <div class="card-header bg-primary text-white"><i class="fas fa-download me-2"></i>Export</div>
          <div class="card-body">
            <p class="small text-muted">Use the form to filter records, then choose an export format.</p>
            <ul class="list-unstyled small">
              ...
              <li><i class="fas fa-file-code me-2 text-info"></i><strong>GeoJSON</strong> -- FeatureCollection for GIS</li>
            </ul>
          </div>
        </div>

        <div class="card mb-3">
          <div class="card-header bg-primary text-white"><i class="fas fa-columns me-2"></i>Output Columns</div>
        ...
        <div class="card mb-3">
          <div class="card-header bg-primary text-white"><i class="fas fa-question-circle me-2"></i>Help</div>
          <div class="card-body small">
            <p>The Spatial Analysis Export tool extracts geographic data from archival descriptions for use in GIS applications.</p>
            <p><strong>CSV export</strong> can be imported into QGIS, ArcGIS, or Google Earth.</p>
            <p><strong>GeoJSON export</strong> produces a FeatureCollection suitable for web mapping libraries like Leaflet or OpenLayers.</p>
          </div>
        </div>
      </div>
    </div>
