        ...
        <tbody>
            @forelse($items as $item)
                <tr>
                    ...
                    <td>
                        @if(!empty($item->entity_iri))
                            <a href="{{ url('/display/' . urlencode($item->entity_iri)) }}" class="btn btn-sm btn-outline-secondary">View</a>
                        @endif
                    </td>
                </tr>
            @empty
                <tr><td colspan="5" class="text-muted text-center">No items in this folder.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
...
@auth
    <div class="mt-3">
        ...
        <form method="post" action="{{ route('favorites.import') }}">
            @csrf
            <input type="hidden" name="iris" value="{{ $items->pluck('entity_iri')->implode("\n") }}">
            <button type="submit" class="btn btn-outline-success">
                <i class="fas fa-heart me-1"></i>Copy to My Favorites
            </button>
        </form>
    </div>
@else
    <div class="alert alert-info mt-3">
        <a href="{{ route('login') }}">Log in</a> to save these to your favorites.
    </div>
@endauth
@endsection
