...
@php
    /**
     * Resolve the display URL for a favorited entity based on its entity_type and entity_iri.
     */
    function favEntityUrl(object $item): string {
        $iri = $item->entity_iri ?? '';
        if (empty($iri)) {
            return '#';
        }
        return url('/display/' . urlencode($iri));
    }

    /**
     * Map entity_type to a Font Awesome icon class.
     */
    function favEntityIcon(object $item): string {
        return match ($item->entity_type ?? 'RecordResource') {
            'Agent'             => 'fas fa-user',
            'CorporateBody'     => 'fas fa-building',
            'Person'            => 'fas fa-user-tie',
            'Family'            => 'fas fa-users',
            'RecordSet'         => 'fas fa-archive',
            'RecordPart'        => 'fas fa-puzzle-piece',
            'Instantiation'     => 'fas fa-copy',
            'Place'             => 'fas fa-map-marker-alt',
            'Activity'          => 'fas fa-tasks',
            'Mandate'           => 'fas fa-gavel',
            'Rule'              => 'fas fa-balance-scale',
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex gap-2 align-items-center">
        ...
        <div class="btn-group btn-group-sm">
            <a href="{{ route('favorites.browse', array_merge($params, ['view' => 'table'])) }}"
               class="btn {{ ($params['view'] ?? 'table') === 'table' ? 'btn-primary' : 'btn-outline-secondary' }}"
               title="Table View">
                <i class="fas fa-list"></i>
            </a>
            <a href="{{ route('favorites.browse', array_merge($params, ['view' => 'grid'])) }}"
               class="btn {{ ($params['view'] ?? 'table') === 'grid' ? 'btn-primary' : 'btn-outline-secondary' }}"
               title="Grid View">
                <i class="fas fa-th"></i>
            </a>
        </div>
...
<div class="row">
    ...
    <div class="col-lg-3 col-md-4">
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center bg-primary text-white">
                ...
                <button class="btn btn-sm btn-outline-light" data-bs-toggle="modal" data-bs-target="#newFolderModal">
                    <i class="fas fa-plus"></i>
                </button>
            </div>
            <div class="list-group list-group-flush">
                <a href="{{ route('favorites.browse') }}"
                   class="list-group-item list-group-item-action d-flex justify-content-between align-items-center{{ empty($params['folder_id']) && empty($params['unfiled']) ? ' active' : '' }}">
                    <span><i class="fas fa-heart me-2"></i>All Favorites</span>
                    <span class="badge bg-primary rounded-pill">{{ $totalCount }}</span>
                </a>
                <a href="{{ route('favorites.browse', ['unfiled' => 1]) }}"
                   class="list-group-item list-group-item-action d-flex justify-content-between align-items-center{{ !empty($params['unfiled']) ? ' active' : '' }}">
                ...
                @if(!empty($folders) && count($folders) > 0)
                    <li class="list-group-item px-3 py-1 bg-light">
                        <small class="text-muted text-uppercase">My Folders</small>
                    </li>
                        ...
                        <a href="{{ route('favorites.browse', ['folder_id' => $folder->id]) }}"
                           class="list-group-item list-group-item-action d-flex justify-content-between align-items-center{{ ($params['folder_id'] ?? '') == $folder->id ? ' active' : '' }}">
                            <span>
                                ...
                                @if($folder->color)
                                    <i class="fas fa-circle me-1" style="color:{{ $folder->color }}; font-size:0.5em;"></i>
                                @endif
                                {{ $folder->name }}
        ...
        {{-- Folder Actions (when viewing a specific folder) --}}
        @if(isset($params['folder_id']) && $params['folder_id'])
            @php $activeFolder = $folders->firstWhere('id', $params['folder_id']); @endphp
            @if($activeFolder)
                <div class="card mb-3">
                    <div class="card-header bg-primary text-white">Folder Actions</div>
                    <div class="card-body py-2">
                        ...
                        <div class="d-flex gap-1 flex-wrap">
                            <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#editFolderModal">
                            ...
                            @else
                                <button type="button" class="btn btn-sm btn-outline-info" data-bs-toggle="modal" data-bs-target="#shareInfoModal">
    ...
    <div class="col-lg-9 col-md-8">
        ...
        <form method="get" action="{{ route('favorites.browse') }}" class="row g-2 mb-3 align-items-end">
            ...
            @endif
            @if(!empty($params['unfiled']))
                <input type="hidden" name="unfiled" value="1">
            @endif
            @if(($params['view'] ?? 'table') !== 'table')
                <input type="hidden" name="view" value="{{ $params['view'] }}">
            @endif

            <div class="col-md-5">
                <div class="input-group input-group-sm">
                    <input type="text" name="query" class="form-control" placeholder="Search favorites..."
                           value="{{ $params['query'] ?? '' }}">
                    <button class="btn btn-outline-secondary" type="submit"><i class="fas fa-search"></i></button>
                    @if(!empty($params['query']))
                        <a href="{{ route('favorites.browse', array_diff_key($params, ['query' => ''])) }}"
                           class="btn btn-outline-danger" title="Clear search"><i class="fas fa-times"></i></a>
                    @endif
                </div>
            </div>
            <div class="col-md-4">
                <select name="sort" class="form-select form-select-sm" onchange="this.form.submit();">
                    @foreach(['created_at' => 'Date Added (Newest)', 'title' => 'Title (A-Z)', 'reference_code' => 'Reference Code (A-Z)', 'type' => 'Entity Type'] as $val => $label)
                ...
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    @if(!empty($folders) && count($folders) > 0)
                        <div class="input-group input-group-sm" style="max-width: 250px;">
                            ...
                            <button type="submit" name="action" value="move" class="btn btn-sm btn-outline-secondary">
        ...
        @if($results->isEmpty())
            <div class="alert alert-info">
                ...
                @else
                    You have no favorites yet. Browse the archive and click the star icon to add items.
                @endif
            </div>
                    ...
                    <div class="col">
                        <div class="card h-100 border">
                            <div class="card-body">
                                ...
                                <h6 class="card-title">
                                    <i class="{{ favEntityIcon($item) }} text-muted me-1"></i>
                                    <a href="{{ favEntityUrl($item) }}" class="text-decoration-none">{{ $item->title }}</a>
                                </h6>
                                @if(!empty($item->reference_code))
                                    <p class="card-text small text-muted mb-1">
                                        <i class="fas fa-barcode me-1"></i>{{ $item->reference_code }}
                                    </p>
                                @endif
                                <p class="card-text small text-muted mb-1">
                ...
                <table class="table table-bordered table-striped table-sm align-middle mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center col-optional col-iri" style="width:200px">Entity IRI</th>
                    ...
                    <tbody>
                        @foreach($results as $item)
                            <tr>
                                <td>
                                    <input type="checkbox" name="ids[]" value="{{ $item->id }}" class="form-check-input bulk-checkbox">
                                </td>
                                <td>
                                    <a href="{{ favEntityUrl($item) }}" class="text-decoration-none">
                                        <i class="{{ favEntityIcon($item) }} text-muted me-2"></i>{{ $item->title }}
                                    </a>
                                </td>
                                <td>
                                    <span class="badge bg-secondary">{{ $item->entity_type ?? '' }}</span>
                                ...
                                <td class="text-center small text-muted text-truncate col-optional col-iri" style="max-width:200px;" title="{{ $item->entity_iri ?? '' }}">
                                    {{ $item->entity_iri ?? '' }}
                                </td>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-outline-secondary notes-toggle"
                                            data-fav-id="{{ $item->id }}" title="Notes">
                                        <i class="fas fa-sticky-note{{ !empty($item->notes) ? ' text-warning' : '' }}"></i>
                                    </button>
                                </td>
                                <td class="text-center">
                                    <a href="{{ favEntityUrl($item) }}" class="btn btn-sm btn-outline-secondary me-1" title="View">
                            ...
                            <tr class="notes-row" id="notes-row-{{ $item->id }}" style="display: none;">
                                ...
                                <td colspan="7">
                                    <div class="input-group input-group-sm">
                                        ...
                                        <button class="btn btn-outline-secondary notes-save" data-fav-id="{{ $item->id }}" type="button">
        ...
        {{-- Pagination --}}
        @if(!$results->isEmpty() && $lastPage > 1)
            <nav aria-label="Favorites pagination" class="mt-3">
                <ul class="pagination justify-content-center">
                    @if($page > 1)
                        <li class="page-item">
                            <a class="page-link" href="{{ route('favorites.browse', array_merge($params, ['page' => $page - 1])) }}">
                    ...
                    @for($p = max(1, $page - 2); $p <= min($lastPage, $page + 2); $p++)
                        <li class="page-item {{ $p === $page ? 'active' : '' }}">
                            <a class="page-link" href="{{ route('favorites.browse', array_merge($params, ['page' => $p])) }}">{{ $p }}</a>
                        </li>
                    @endfor
        ...
        <form method="post" action="{{ route('favorites.folder.create') }}">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-folder-plus me-2"></i>New Folder</h5>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <form method="post" action="{{ route('favorites.import') }}" enctype="multipart/form-data">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-upload me-2"></i>Import Favorites</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <small class="text-muted">CSV should contain one entity IRI per line.</small>
                    </div>
                    <div class="text-center text-muted my-2">&mdash; or &mdash;</div>
                    <div class="mb-3">
                        <label class="form-label">Paste Entity IRIs</label>
                        <textarea name="iris" class="form-control" rows="4"
                                  placeholder="One entity IRI per line..."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
        ...
        <form method="post" action="{{ route('favorites.folder.edit', $activeFolder->id) }}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Folder Name <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control" value="{{ e($activeFolder->name) }}" required maxlength="255">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Description</label>
                        <textarea name="description" class="form-control" rows="2">{{ e($activeFolder->description ?? '') }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Color</label>
                        <input type="color" name="color" class="form-control form-control-color" value="{{ $activeFolder->color ?? '#0d6efd' }}">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
...
<div class="modal fade" id="shareInfoModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-share-alt me-2"></i>Folder is Shared</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" readonly
                               value="{{ url('/favorites/shared/' . $activeFolder->share_token) }}" id="shareLinkInput">
                        <button type="button" class="btn btn-outline-secondary" id="copyShareLinkBtn"
                                onclick="navigator.clipboard.writeText(document.getElementById('shareLinkInput').value); this.innerHTML='<i class=\'fas fa-check\'></i> Copied';">
            ...
            <div class="modal-footer">
                <form action="{{ route('favorites.folder.revoke', $activeFolder->id) }}" method="post" class="d-inline">
                    ...
                    <button type="submit" class="btn btn-outline-danger"
                            onclick="return confirm('Revoke sharing? The link will no longer work.');">
...
{{-- ─────────── Column Config (floating button) ─────────── --}}
<div class="dropdown position-fixed" style="bottom: 20px; right: 20px; z-index: 1050;">
    <button class="btn btn-sm btn-outline-secondary rounded-circle shadow" type="button" data-bs-toggle="dropdown"
            title="Column Settings" style="width:36px;height:36px;">
    ...
    <ul class="dropdown-menu dropdown-menu-end">
        ...
        <li>
            <label class="dropdown-item">
                <input type="checkbox" class="form-check-input me-2 col-toggle" data-col="col-iri" checked>
                Entity IRI
            </label>
        </li>
    </ul>
