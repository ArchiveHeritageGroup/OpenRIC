...
@section('content')
  ...
  <form action="{{ route('search') }}" method="get">
    <div class="card mb-4">
      <div class="card-header" style="background:var(--bs-primary, #0d6efd);color:#fff"><strong>Search criteria</strong></div>
      <div class="card-body">
        <div class="mb-3">
          <label for="adv-query" class="form-label fw-semibold">Search terms</label>
          <input type="text" id="adv-query" name="q" class="form-control form-control-lg" placeholder="Enter keywords..." value="{{ $query }}" autocomplete="off">
          <div class="form-text">Searches title, scope and content, identifier, reference code, and creator names.</div>
        </div>
      ...
      <div class="card-footer d-flex gap-2">
        <button type="submit" class="btn atom-btn-outline-success"><i class="fas fa-search"></i> Search</button>
        <a href="{{ route('search.advanced') }}" class="btn atom-btn-white"><i class="fas fa-undo"></i> Reset</a>
        <a href="{{ route('search') }}" class="btn atom-btn-white ms-auto"><i class="fas fa-arrow-left"></i> Back to simple search</a>
      </div>
    </div>
  </form>
@endsection
