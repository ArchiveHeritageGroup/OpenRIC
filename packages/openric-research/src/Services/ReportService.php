 ...
 *
 * Adapted from Heratio AhgResearch\Services\ReportService.
...
class ReportService
{
    ...
    public function createFromTemplate(int $researcherId, string $templateCode, array $data): int
    {
        ...
        if ($template) {
            ...
            foreach ($sections as $sectionDef) {
                ...
                $validTypes = ['title_page', 'toc', 'heading', 'text', 'bibliography', 'collection_list', 'annotation_list', 'timeline', 'custom'];

                DB::table('research_report_section')->insert([
                    'report_id'      => $reportId,
                    'section_type'   => in_array($type, $validTypes, true) ? $type : 'text',
                    'title'          => $title,
                    'content'        => '',
                    'content_format' => 'html',
                    'sort_order'     => $order++,
    ...
    public function reorderSections(int $reportId, array $sectionIds): void
    {
        ...
        DB::table('research_report')->where('id', $reportId)->update(['updated_at' => now()]);
    }
