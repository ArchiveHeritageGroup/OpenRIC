 ...
 *
 * Handles research projects, collaboration, milestones, resources,
 * clipboard integration, and activity tracking.
 *
 * Adapted from Heratio AhgResearch\Services\ProjectService.
...
class ProjectService
{
    ...
    public function getProject(int $projectId): ?object
    {
        ...
        if ($project) {
            ...
            $project->resource_count = (int) DB::table('research_project_resource')
                ->where('project_id', $projectId)
                ->count();
            $project->milestone_count = (int) DB::table('research_project_milestone')
                ->where('project_id', $projectId)
                ->count();
            $project->completed_milestones = (int) DB::table('research_project_milestone')
                ->where('project_id', $projectId)
    ...
    public function getProjects(int $researcherId, array $filters = []): array
    {
        ...
        foreach ($projects as &$project) {
            $project->collaborator_count = (int) DB::table('research_project_collaborator')
    ...
    public function updateProject(int $projectId, array $data): bool
    {
        ...
        foreach (['start_date', 'expected_end_date', 'actual_end_date'] as $dateField) {
    ...
    {
        $project = DB::table('research_project')->where('id', $projectId)->first();
        if ($project && (int) $project->owner_id === $researcherId) {
            return false;
        }
    ...
    public function getCollaborators(int $projectId, bool $acceptedOnly = false): array
    {
        ...
        return $query->orderByRaw("CASE pc.role WHEN 'owner' THEN 1 WHEN 'editor' THEN 2 WHEN 'contributor' THEN 3 WHEN 'viewer' THEN 4 ELSE 5 END")
            ->get()
            ->toArray();
    }
    ...
    {
        $maxOrder = (int) (DB::table('research_project_resource')
            ->where('project_id', $projectId)
            ->max('sort_order') ?? 0);
    ...
    {
        $maxOrder = (int) (DB::table('research_project_milestone')
            ->where('project_id', $projectId)
            ->max('sort_order') ?? 0);
    ...
    public function toggleClipboardPin(int $itemId, int $researcherId): bool
    {
        ...
        return DB::table('research_clipboard_project')
            ->where('id', $itemId)
            ->update(['is_pinned' => $item->is_pinned ? false : true]) >= 0;
    }
