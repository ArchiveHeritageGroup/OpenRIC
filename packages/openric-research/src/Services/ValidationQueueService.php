 ...
 *
 * Adapted from Heratio AhgResearch\Services\ValidationQueueService.
...
class ValidationQueueService
{
    public function getQueue(?int $researcherId = null, array $filters = [], int $page = 1, int $limit = 25): array
    {
        $query = DB::table('research_validation_queue as vq')
            ...
            ->select(
                'vq.id', 'vq.result_id', 'vq.researcher_id', 'vq.status',
                'vq.reviewer_id', 'vq.reviewed_at', 'vq.notes',
                'vq.modified_data_json', 'vq.created_at',
                'er.object_id', 'er.result_type', 'er.data_json',
                'er.confidence', 'er.model_version', 'er.job_id',
                'ej.extraction_type', 'ej.project_id',
