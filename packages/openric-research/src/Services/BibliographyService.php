 ...
 * Also handles import from BibTeX and RIS, and citation auto-generation.
 *
 * Adapted from Heratio AhgResearch\Services\BibliographyService.
...
class BibliographyService
{
    ...
    public function getBibliographies(int $researcherId, ?int $projectId = null): array
    {
        ...
        foreach ($bibliographies as &$bib) {
            $bib->entry_count = (int) DB::table('research_bibliography_entry')
    ...
    {
        $typeMap = [
            'archival' => 'UNPB', 'book' => 'BOOK', 'article' => 'JOUR',
            'chapter' => 'CHAP', 'thesis' => 'THES', 'website' => 'ELEC', 'other' => 'GEN',
        ];
    ...
    {
        $typeMap = [
            'archival' => 'misc', 'book' => 'book', 'article' => 'article',
            'chapter' => 'incollection', 'thesis' => 'phdthesis', 'website' => 'online', 'other' => 'misc',
        ];
    ...
    {
        $map = [
            'archival' => 'generic', 'book' => 'book', 'article' => 'journal',
            'chapter' => 'book_section', 'thesis' => 'thesis', 'website' => 'web_page', 'other' => 'generic',
        ];
        ...
        foreach ($entries as $entry) {
            ...
            if ($entry->authors) {
                ...
                foreach (explode(';', $entry->authors) as $author) {
                    ...
                    if (count($parts) === 2) {
                        $item['author'][] = ['family' => $parts[0], 'given' => $parts[1]];
                    } else {
    ...
    {
        $map = [
            'archival' => 'manuscript', 'book' => 'book', 'article' => 'article-journal',
            'chapter' => 'chapter', 'thesis' => 'thesis', 'website' => 'webpage', 'other' => 'document',
        ];
    ...
    protected function formatChicago(object $record): string
    {
        ...
        if ($record->date_display) {
            $parts[] = $record->date_display;
        }
        if ($record->ref_code) {
            $parts[] = $record->ref_code;
        }
        if ($record->repository_name) {
            $parts[] = $record->repository_name;
        }
    ...
    protected function formatIsadg(object $record): string
    {
        ...
        if ($record->extent_and_medium) {
            $parts[] = $record->extent_and_medium;
        }
