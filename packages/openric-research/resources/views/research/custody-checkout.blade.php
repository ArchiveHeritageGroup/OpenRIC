{{-- Custody Checkout - Adapted from Heratio --}}
...
<div class="card">
    <div class="card-body">
        <form method="POST">
            @csrf
            <div class="row mb-3">
                <div class="col-md-6"><label class="form-label">Item <span class="badge bg-danger ms-1">Required</span></label><input type="text" name="item_title" class="form-control" value="{{ e($item->title ?? '') }}" readonly></div>
                <input type="hidden" name="item_id" value="{{ $item->id ?? 0 }}">
                <div class="col-md-6"><label class="form-label">Researcher <span class="badge bg-danger ms-1">Required</span></label><input type="text" class="form-control" value="{{ e(($researcher->first_name ?? '') . ' ' . ($researcher->last_name ?? '')) }}" readonly></div>
            ...
            <button type="submit" class="btn btn-outline-primary"><i class="fas fa-check me-1"></i>Confirm Checkout</button>
