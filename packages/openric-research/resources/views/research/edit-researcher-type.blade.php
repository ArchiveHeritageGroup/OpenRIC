{{-- Edit Researcher Type - Adapted from Heratio --}}
@extends('theme::layouts.1col')
@section('title', ($isNew ?? true) ? 'Add Researcher Type' : 'Edit Researcher Type')
@section('content')
<div class="d-flex">
    @include('research::research._sidebar', ['sidebarActive' => 'adminTypes'])
...
<div class="card">
    <div class="card-body">
        <form method="POST">
            @csrf
            <div class="row mb-3">
                <div class="col-md-6"><label class="form-label">Name <span class="badge bg-danger ms-1">Required</span></label><input type="text" name="name" class="form-control" required value="{{ e($type->name ?? '') }}"></div>
                <div class="col-md-3"><label class="form-label">Code <span class="badge bg-danger ms-1">Required</span></label><input type="text" name="code" class="form-control" required value="{{ e($type->code ?? '') }}"></div>
            ...
            <button type="submit" class="btn btn-outline-primary"><i class="fas fa-save me-1"></i>Save</button>
            <a href="{{ route('research.adminTypes') }}" class="btn btn-outline-secondary">Cancel</a>
        </form>
    </div>
</div>
    </div>
