{{-- Accessibility Helpers - Adapted from Heratio --}}
...
(function() {
    'use strict';
    window.openricAnnounce = function(message, priority) {
    ...
    window.openricFocusTo = function(selector) {
        var el = document.querySelector(selector);
        if (el) { el.setAttribute('tabindex', '-1'); el.focus(); }
    };
    if (!document.getElementById('openricA11yStyles')) {
        var style = document.createElement('style');
        style.id = 'openricA11yStyles';
        style.textContent =
            '*:focus-visible { outline: 3px solid #0d6efd !important; outline-offset: 2px; }' +
            '.openric-sr-only { position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden; clip: rect(0,0,0,0); white-space: nowrap; border: 0; }';
