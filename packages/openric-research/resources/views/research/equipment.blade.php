...
<div class="d-flex">
    ...
    <div class="flex-grow-1 p-4">
        ...
        <form method="GET" action="{{ route('research.equipment') }}" class="mb-3">
        ...
        @if($currentRoom)
            <h5>{{ $currentRoom->name }} - Equipment</h5>
            <div class="table-responsive">
                <table class="table table-striped">
                    <thead><tr><th>Name</th><th>Type</th><th>Status</th><th>Notes</th></tr></thead>
                    <tbody>
                    @forelse($equipment as $e)
                        <tr><td>{{ $e->name }}</td><td>{{ $e->equipment_type ?? '-' }}</td><td><span class="badge bg-{{ ($e->is_available ?? true) ? 'success' : 'warning' }}">{{ ($e->is_available ?? true) ? 'Available' : 'In Use' }}</span></td><td>{{ $e->notes ?? '-' }}</td></tr>
                    @empty
                        <tr><td colspan="4" class="text-muted text-center">No equipment for this room.</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
