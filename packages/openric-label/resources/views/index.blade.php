{{--
    Label index view — adapted from Heratio ahg-label resources/views/index.blade.php (187 lines).

    Heratio differences:
      - Uses ahg-theme-b5::layout → OpenRiC uses theme::layouts.1col
      - Slug-based URLs → IRI-based URLs with base64 encoding
      - Same label configuration UI: barcode source dropdown, label size selector,
        show/hide toggles for barcode, QR, title, repository
      - Same preview panel with live barcode/QR rendering via external APIs
      - Same print/download actions
      - Bootstrap 5 throughout
--}}
@extends('theme::layouts.1col')

@section('title', 'Print Labels: ' . e($title ?: $identifier))
...
<h1 class="no-print">Print Labels: {{ e($title ?: $identifier) }}</h1>

<div class="row">
    {{-- Configuration panel — adapted from Heratio's col-md-8 config card --}}
    <div class="col-md-8 no-print">
        <div class="card mb-3">
            <div class="card-header">
                <i class="fas fa-cog me-2"></i>Label Configuration
                <span class="badge bg-secondary ms-2">{{ e($sectorLabel) }}</span>
            </div>
            <div class="card-body">
                <div class="row">
                    {{-- Barcode Source Dropdown — same as Heratio --}}
        ...
        {{-- Action Buttons — adapted from Heratio: back links to record IRI instead of slug --}}
        <div class="mb-3">
            <a class="btn btn-outline-secondary" href="{{ e($qrUrl) }}">
    ...
    {{-- Preview panel — same structure as Heratio --}}
    <div class="col-md-4">
        <div class="card">
            <div class="card-header no-print">Preview</div>
...
{{--
    JavaScript — same as Heratio with identical toggle/update functions.
    downloadLabel() uses html2canvas if available, same fallback alert.
--}}
<script>
...
function downloadLabel() {
    if (typeof html2canvas !== 'undefined') {
        html2canvas(document.getElementById('labelContent')).then(function(canvas) {
            ...
            link.download = 'label-{{ e($identifier ?: "entity") }}.png';
