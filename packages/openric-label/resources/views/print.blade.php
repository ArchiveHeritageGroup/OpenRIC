{{--
    Batch print view — adapted from Heratio ahg-label resources/views/batch.blade.php (70 lines).

    Heratio differences:
      - Uses ahg-theme-b5::layout → OpenRiC uses theme::layouts.1col
      - Slug-based entity references → IRI-based
      - Same label grid layout with flex-wrap
      - Same print styles with page-break-inside: avoid
      - Same barcode/QR rendering via external APIs
...
<div class="d-flex flex-wrap justify-content-center">
    @foreach ($labels as $label)
        <div class="label-preview" style="max-width: {{ e($labelSize) }}px;">
            @if ($showTitle)
                <div class="fw-bold mb-2" style="font-size: 11pt;">
                    {{ e($label['title'] ?: $label['identifier'] ?? '') }}
                </div>
            @endif
