...
@section('title', 'Edit Repository Permissions')
@section('body-class', 'view repo-acl')
...
<div class="container mt-4">
    <h1>Edit Repository Permissions for {{ $userName }}</h1>
    
    @if(session('success'))
        <div class="alert alert-success">{{ session('success') }}</div>
    @endif
    
    <p class="text-muted mb-3">Grant or deny specific repository permissions for this user.</p>
    
    <form method="POST" action="{{ route('user.editRepositoryAcl', $userSlug) }}">
        @csrf
        ...
        @else
            <p class="text-muted">No specific repository permissions currently set.</p>
        @endif
        ...
        <div class="row g-3">
            <div class="col-md-4">
                <label for="new_repo_id" class="form-label">Repository</label>
                <select name="new_repo_id" id="new_repo_id" class="form-select">
                    <option value="">-- Select Repository --</option>
                    @foreach($repoList as $repo)
                        <option value="{{ is_array($repo) ? $repo['id'] : $repo->id }}">
        ...
        <div class="mt-4">
            ...
            <a href="{{ route('user.indexRepositoryAcl', $userSlug) }}" class="btn btn-secondary">Cancel</a>
