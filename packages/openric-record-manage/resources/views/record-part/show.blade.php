...
@section('title', $entity['properties']['rico:title'][0]['value'] ?? 'Record Part')
...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3 mb-0">{{ $entity['properties']['rico:title'][0]['value'] ?? 'Record Part' }}</h1>
        <div class="btn-group">
            <a href="{{ route('record-parts.edit', ['iri' => urlencode($entity['iri'])]) }}" class="btn btn-outline-primary btn-sm">Edit</a>
            <form method="POST" action="{{ route('record-parts.destroy', ['iri' => urlencode($entity['iri'])]) }}" class="d-inline" onsubmit="return confirm('Delete this Record Part?')">
                @csrf
                @method('DELETE')
