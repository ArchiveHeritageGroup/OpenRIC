 ...
 *
 * Adapted from Heratio AhgHeritageManage\Services\HeritageSearchService.
 * PostgreSQL ILIKE used instead of MySQL LIKE for case-insensitive search.
 */
class HeritageSearchService
{
    ...
    public function search(string $query, array $filters = [], int $page = 1, int $limit = 10): array
    {
        ...
        if (!empty($terms)) {
            $baseQuery->orderByRaw("CASE WHEN ioi.title ILIKE ? THEN 0 ELSE 1 END", ['%' . $terms[0] . '%']);
        }
    ...
    private function buildBaseQuery(): \Illuminate\Database\Query\Builder
    {
        return DB::table('information_object as io')
            ...
            ->select(
                ...
                'ioi.arrangement',
                DB::raw("(SELECT EXTRACT(YEAR FROM ev.start_date) FROM event ev WHERE ev.object_id = io.id LIMIT 1) as io_start_year"),
                DB::raw("(SELECT EXTRACT(YEAR FROM ev.end_date) FROM event ev WHERE ev.object_id = io.id LIMIT 1) as io_end_year"),
        ...
        $query->where(function ($q) use ($terms) {
            foreach ($terms as $term) {
                ...
                $q->orWhere(function ($inner) use ($pattern) {
                    $inner->where('ioi.title', 'ILIKE', $pattern)
                        ->orWhere('ioi.scope_and_content', 'ILIKE', $pattern)
                        ->orWhere('ioi.alternate_title', 'ILIKE', $pattern)
                        ->orWhere('ioi.archival_history', 'ILIKE', $pattern)
                        ->orWhere('ioi.arrangement', 'ILIKE', $pattern)
                        ...
                        ->orWhereExists(function ($sub) use ($pattern) {
                            $sub->select(DB::raw(1))
                                ...
                                ->where('ti_kw.name', 'ILIKE', $pattern);
                        })
                        ->orWhereExists(function ($sub) use ($pattern) {
                            $sub->select(DB::raw(1))
                                ...
                                ->where('ai_kw.authorized_form_of_name', 'ILIKE', $pattern);
                        });
                });
            }
        });
    ...
    {
        $results = [];
        $uploadsPath = config('openric.uploads_path', '/usr/share/nginx/archive');
        ...
        foreach ($rows as $row) {
            ...
            $results[] = [
                ...
                'date'       => $this->formatDateRange($row->io_start_year !== null ? (int) $row->io_start_year : null, $row->io_end_year !== null ? (int) $row->io_end_year : null),
    ...
    /**
     * Format a date range from start/end years.
     */
    private function formatDateRange(?int $startYear, ?int $endYear): string
    {
        ...
        $startStr = str_pad((string) abs($start), 4, '0', STR_PAD_LEFT);
        $endStr = str_pad((string) abs($end), 4, '0', STR_PAD_LEFT);
                    ...
                    $q->where('ioi.title', 'ILIKE', $pattern)
                        ...
                        ->orWhereExists(function ($sub) use ($pattern) {
                            $sub->select(DB::raw(1))
                                ...
                                ->where('ti_tm.name', 'ILIKE', $pattern);
                        })
                        ->orWhereExists(function ($sub) use ($pattern) {
                            $sub->select(DB::raw(1))
                                ...
                                ->where('ai_tm.authorized_form_of_name', 'ILIKE', $pattern);
                        });
                })
                ->count();
    ...
    /**
     * Build facets from ALL published items.
     */
