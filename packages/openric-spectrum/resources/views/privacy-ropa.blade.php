        ...
        @if(!empty($activities))
            ...
            <table class="table table-striped">
                ...
                @foreach($activities as $a)
                    @php $a = is_object($a) ? $a : (object)$a; @endphp
                    <tr>
                        ...
                        <td>
                            @if($a->dpia_required ?? false)
                                <span class="badge bg-{{ ($a->dpia_completed ?? false) ? 'success' : 'warning' }}">
                                    {{ ($a->dpia_completed ?? false) ? __('Complete') : __('Required') }}
                                </span>
                            @else
...
<div class="modal fade" id="addRopaModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="post" action="{{ route('spectrum.privacy-ropa') }}">
                @csrf
...
function editRopa(id) {
    window.location.href = '{{ route('spectrum.privacy-ropa') }}?edit=' + id;
}
function deleteRopa(id) {
    if (confirm('{{ __('Delete this processing activity?') }}')) {
        window.location.href = '{{ route('spectrum.privacy-ropa') }}?delete=' + id;
    }
}
</script>
@endsection
