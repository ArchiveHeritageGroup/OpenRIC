...
function safeSpectrumLabelQuery($table, $objectId, $column) {
    try {
...
$isbn = safeSpectrumLabelQuery('library_item', $objectId, 'isbn');
if (!empty($isbn)) {
    $barcodeSources['isbn'] = [
        'label' => __('ISBN'),
        'value' => $isbn,
...
$issn = safeSpectrumLabelQuery('library_item', $objectId, 'issn');
if (!empty($issn)) {
    $barcodeSources['issn'] = [
        'label' => __('ISSN'),
        'value' => $issn,
...
$lccn = safeSpectrumLabelQuery('library_item', $objectId, 'lccn');
if (!empty($lccn)) {
    $barcodeSources['lccn'] = [
        'label' => __('LCCN'),
        'value' => $lccn,
...
$openlibrary = safeSpectrumLabelQuery('library_item', $objectId, 'openlibrary_id');
...
$barcode = safeSpectrumLabelQuery('library_item', $objectId, 'barcode');
if (!empty($barcode)) {
...
$callNumber = safeSpectrumLabelQuery('library_item', $objectId, 'call_number');
if (!empty($callNumber)) {
...
$accession = safeSpectrumLabelQuery('museum_object', $objectId, 'accession_number');
if (!empty($accession)) {
...
$objectNumber = safeSpectrumLabelQuery('museum_object', $objectId, 'object_number');
