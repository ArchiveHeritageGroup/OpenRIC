...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('spectrum.dashboard') }}">{{ __('Spectrum') }}</a></li>
...
<div class="row">
    <div class="col-md-6 mb-4">
        <div class="card h-100">
            ...
            <div class="card-body"><h3>{{ $counts['movements'] ?? 0 }}</h3><p class="text-muted">{{ __('records available') }}</p></div>
        </div>
    </div>
    <div class="col-md-6 mb-4">
        <div class="card h-100">
            ...
            <div class="card-body"><h3>{{ $counts['conditions'] ?? 0 }}</h3><p class="text-muted">{{ __('records available') }}</p></div>
        </div>
    </div>
    <div class="col-md-4 mb-4">
        <div class="card h-100">
            <div class="card-header bg-warning"><h5 class="mb-0"><i class="fas fa-dollar-sign me-2"></i>{{ __('Valuations') }}</h5></div>
            <div class="card-body"><h3>{{ $counts['valuations'] ?? 0 }}</h3><p class="text-muted">{{ __('records available') }}</p></div>
        </div>
    </div>
    <div class="col-md-4 mb-4">
        <div class="card h-100">
            <div class="card-header bg-info text-white"><h5 class="mb-0"><i class="fas fa-sign-in-alt me-2"></i>{{ __('Loans In') }}</h5></div>
            <div class="card-body"><h3>{{ $counts['loansIn'] ?? 0 }}</h3><p class="text-muted">{{ __('records available') }}</p></div>
        </div>
    </div>
    <div class="col-md-4 mb-4">
        <div class="card h-100">
            <div class="card-header bg-secondary text-white"><h5 class="mb-0"><i class="fas fa-sign-out-alt me-2"></i>{{ __('Loans Out') }}</h5></div>
            <div class="card-body"><h3>{{ $counts['loansOut'] ?? 0 }}</h3><p class="text-muted">{{ __('records available') }}</p></div>
        </div>
    </div>
</div>
        ...
        <form method="get" action="" class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <select name="type" class="form-select">
                    <option value="movement">{{ __('Movements') }} ({{ $counts['movements'] ?? 0 }})</option>
                    <option value="condition">{{ __('Condition Checks') }} ({{ $counts['conditions'] ?? 0 }})</option>
                    <option value="valuation">{{ __('Valuations') }} ({{ $counts['valuations'] ?? 0 }})</option>
                    <option value="loan">{{ __('Loans') }} ({{ ($counts['loansIn'] ?? 0) + ($counts['loansOut'] ?? 0) }})</option>
