...
<div class="modal fade" id="addDsarModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="post" action="{{ route('spectrum.privacy-dsar') }}">
                @csrf
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        {{ __('POPIA requires response within') }} <strong>30 {{ __('days') }}</strong>. {{ __('Deadline will be calculated automatically.') }}
                    </div>
                    <div class="row">
...
function updateDsarStatus(id, status) {
    if (confirm('{{ __('Update status to') }} ' + status + '?')) {
        fetch('{{ route('spectrum.privacy-dsar') }}/update', {
            method: 'POST',
            headers: {
