        ...
        <div class="card mb-3">
            ...
            <ul class="list-group list-group-flush">
                <li class="list-group-item">
                    <a href="{{ route('spectrum.label') }}?slug={{ $resource->slug ?? '' }}">
                        <i class="fas fa-barcode me-2"></i>{{ __('Print Labels') }}
                    </a>
                </li>
                <li class="list-group-item">
                    <a href="{{ route('spectrum.condition-photos') }}?slug={{ $resource->slug ?? '' }}">
                        <i class="fas fa-camera me-2"></i>{{ __('Condition Photos') }}
                    </a>
                </li>
                <li class="list-group-item">
                    <a href="{{ route('spectrum.grap-dashboard') }}?slug={{ $resource->slug ?? '' }}">
                    ...
                    <div class="col-lg-3 col-md-4 col-sm-6">
                        <div class="card h-100 border-{{ $color }}">
                            <div class="card-body text-center p-3">
                                ...
                                <a href="{{ route('spectrum.workflow') }}?slug={{ $resource->slug ?? '' }}&procedure_type={{ $key }}"
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="btn-group flex-wrap" role="group">
                    <a href="{{ route('spectrum.export') }}?slug={{ $resource->slug ?? '' }}&format=pdf" class="btn btn-outline-danger">
                        <i class="fas fa-file-pdf me-1"></i>{{ __('Export PDF') }}
                    </a>
                    <a href="{{ route('spectrum.export') }}?slug={{ $resource->slug ?? '' }}&format=csv" class="btn btn-outline-success">
                        <i class="fas fa-file-csv me-1"></i>{{ __('Export CSV') }}
                    </a>
                    <a href="{{ route('spectrum.export') }}?slug={{ $resource->slug ?? '' }}&format=json" class="btn btn-outline-primary">
