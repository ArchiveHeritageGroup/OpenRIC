...
<div class="mb-3">
    <a href="{{ route('spectrum.privacy-admin') }}" class="btn btn-secondary">{{ __('Back') }}</a>
        ...
        @if(!empty($breaches))
            ...
            <table class="table table-striped">
                ...
                @foreach($breaches as $b)
                    @php $b = is_object($b) ? $b : (object)$b; @endphp
                    <tr>
...
<div class="modal fade" id="reportBreachModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="post" action="{{ route('spectrum.privacy-breaches') }}">
                @csrf
                ...
                <div class="modal-body">
                    <div class="alert alert-warning">
                        ...
                        {{ __('POPIA requires notification to the Information Regulator within') }} <strong>72 {{ __('hours') }}</strong> {{ __('if the breach poses a risk to data subjects.') }}
                    </div>
                    <div class="row">
...
function closeBreach(id) {
    if (confirm('{{ __('Close this breach incident?') }}')) {
        fetch('{{ route('spectrum.privacy-breaches') }}/update', {
            method: 'POST',
            headers: {
...
function notifyRegulator(id) {
    if (confirm('{{ __('Mark as notified to Information Regulator?') }}')) {
        fetch('{{ route('spectrum.privacy-breaches') }}/update', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
