...
$wfConfig = DB::table('spectrum_workflow_config')
    ->where('procedure_type', $procedureType)
    ->where('is_active', 1)
    ->first();

$configData = $wfConfig ? json_decode($wfConfig->config_json, true) : null;
...
$currentStateRow = DB::table('spectrum_workflow_state')
    ->where('record_id', $resource->id ?? 0)
    ->where('procedure_type', $procedureType)
    ->first();

$currentStateName = $currentStateRow->current_state ?? ($configData['initial_state'] ?? 'pending');
...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('spectrum.index') }}?slug={{ $resource->slug ?? '' }}">{{ __('Spectrum') }}</a></li>
        ...
        <div class="card mb-4">
            ...
            <ul class="list-group list-group-flush">
                ...
                <li class="list-group-item {{ $isActive ? 'active' : '' }}">
                    <a href="{{ route('spectrum.workflow') }}?slug={{ $resource->slug ?? '' }}&procedure_type={{ $procId }}"
    ...
    <div class="col-md-9">
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ $wfConfig->name }}</h5>
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <form method="post" action="{{ route('spectrum.workflow') }}" class="row g-3">
                        @csrf
