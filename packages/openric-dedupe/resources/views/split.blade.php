...
@section('content')
  ...
  <div class="card"><div class="card-header fw-semibold" style="background:var(--bs-primary, #0d6efd);color:#fff"><i class="fas fa-edit me-2"></i>Split Authority</div>
  <div class="card-body"><form method="POST" action="{{ $formAction ?? '#' }}">@csrf
    <div class="mb-3"><label class="form-label">Records to Split <span class="badge bg-danger ms-1">Required</span></label><textarea class="form-control" name="split_names" rows="4" placeholder="One name per line"></textarea></div>
