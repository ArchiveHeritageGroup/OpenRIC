...
@section('content')
  ...
  <div class="card"><div class="card-header fw-semibold" style="background:var(--bs-primary, #0d6efd);color:#fff"><i class="fas fa-edit me-2"></i>Authority Configuration</div>
  <div class="card-body"><form method="POST" action="{{ $formAction ?? '#' }}">@csrf
    <div class="mb-3"><label class="form-label">Auto-Scan Interval (days)</label><input type="number" class="form-control" name="auto_scan_interval"></div>
    <div class="mb-3"><label class="form-label">Min Confidence (%)</label><input type="number" class="form-control" name="min_confidence"></div>
