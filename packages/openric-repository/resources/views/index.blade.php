  ...
  <div class="multiline-header d-flex align-items-center mb-3">
    ...
    <div class="d-flex flex-column">
      <h1 class="mb-0" aria-describedby="heading-label">
        @if($result['total'] > 0)
          Showing {{ number_format($result['total']) }} results
        @else
          No results found
        @endif
      </h1>
...
@section('before-content')
  ...
  <div class="d-inline-block mb-3">
    <form method="get" action="{{ url('/repository/browse') }}" class="d-flex gap-2">
      @foreach(request()->except(['subquery', 'page']) as $hk => $hv)
        @if($hv !== '' && $hv !== null)
          <input type="hidden" name="{{ $hk }}" value="{{ $hv }}">
        @endif
      @endforeach
      <input type="text" name="subquery" class="form-control" placeholder="Search {{ mb_strtolower(config('app.ui_label_repository', 'Archival institution')) }}..." value="{{ request('subquery', '') }}">
      <button type="submit" class="btn atom-btn-outline-light"><i class="fas fa-search" aria-hidden="true"></i> Search</button>
    </form>
  </div>
...
@section('content')
  ...
  <div class="d-flex flex-wrap gap-2 mb-3">
    ...
    <div class="d-flex flex-wrap gap-2 ms-auto">
      ...
      <div class="dropdown d-inline-block">
        @php $currentSort = request('sort', 'lastUpdated'); @endphp
        <button class="btn btn-sm atom-btn-white dropdown-toggle text-wrap" type="button" id="sort-button" data-bs-toggle="dropdown" aria-expanded="false">
          Sort by: {{ $sortOptions[$currentSort] ?? 'Date modified' }}
        </button>
        <ul class="dropdown-menu dropdown-menu-end mt-2" aria-labelledby="sort-button">
          @foreach($sortOptions as $sortKey => $sortLabel)
            <li><a href="{{ request()->url() }}?{{ http_build_query(array_merge(request()->except(['sort', 'page']), ['sort' => $sortKey])) }}" class="dropdown-item {{ $currentSort === $sortKey ? 'active' : '' }}">{{ $sortLabel }}</a></li>
          @endforeach
        </ul>
      </div>
  ...
  @if($result['total'] > 0)
    ...
    @else
      <div class="table-responsive mb-3">
        <table class="table table-bordered mb-0">
          <thead>
            <tr>
              <th class="sortable w-40">Name</th>
              <th class="sortable w-20">Region</th>
              <th class="sortable w-20">Locality</th>
    ...
    {{-- Pagination --}}
    @php
      $totalPages = (int) ceil($result['total'] / max(1, $result['limit']));
    ...
    @if($totalPages > 1)
      <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
          @if($currentPage > 1)
            <li class="page-item"><a class="page-link" href="{{ request()->url() }}?{{ http_build_query(array_merge(request()->except('page'), ['page' => $currentPage - 1])) }}">Previous</a></li>
          @endif
          @for($p = max(1, $currentPage - 3); $p <= min($totalPages, $currentPage + 3); $p++)
            <li class="page-item {{ $p == $currentPage ? 'active' : '' }}"><a class="page-link" href="{{ request()->url() }}?{{ http_build_query(array_merge(request()->except('page'), ['page' => $p])) }}">{{ $p }}</a></li>
          @endfor
          @if($currentPage < $totalPages)
            <li class="page-item"><a class="page-link" href="{{ request()->url() }}?{{ http_build_query(array_merge(request()->except('page'), ['page' => $currentPage + 1])) }}">Next</a></li>
          @endif
        </ul>
      </nav>
    @endif
