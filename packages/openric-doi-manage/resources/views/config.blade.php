      ...
      <div class="col-lg-8">
        ...
        <div class="card mb-4">
          <div class="card-header fw-bold bg-primary text-white">
            <i class="fas fa-key me-2"></i>DataCite API Credentials
          </div>
          <div class="card-body">
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <input type="text" class="form-control" id="datacite_repository_id" name="datacite_repository_id"
                       value="{{ old('datacite_repository_id', $settings['repository_id'] ?? '') }}"
            ...
            <div class="row mb-3">
              <label for="datacite_prefix" class="col-sm-3 col-form-label">DOI Prefix</label>
              <div class="col-sm-9">
                <input type="text" class="form-control" id="datacite_prefix" name="datacite_prefix"
                       value="{{ old('datacite_prefix', $settings['prefix'] ?? '') }}"
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <input type="password" class="form-control" id="datacite_password" name="datacite_password"
                       value="{{ old('datacite_password', $settings['password'] ?? '') }}">
                <div class="form-text">DataCite API password</div>
              </div>
            </div>

            <div class="row mb-3">
              <label for="datacite_url" class="col-sm-3 col-form-label">API URL</label>
              <div class="col-sm-9">
                <select class="form-select" id="datacite_url" name="datacite_url">
                  <option value="https://api.datacite.org" {{ (old('datacite_url', $settings['url'] ?? '') === 'https://api.datacite.org') ? 'selected' : '' }}>
                    Production (https://api.datacite.org)
                  </option>
                  <option value="https://api.test.datacite.org" {{ (old('datacite_url', $settings['url'] ?? '') === 'https://api.test.datacite.org') ? 'selected' : '' }}>
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <select class="form-select" id="datacite_environment" name="datacite_environment">
                  <option value="test" {{ (old('datacite_environment', $settings['environment'] ?? '') === 'test') ? 'selected' : '' }}>
                    Test
                  </option>
                  <option value="production" {{ (old('datacite_environment', $settings['environment'] ?? '') === 'production') ? 'selected' : '' }}>
                    Production
                  </option>
                </select>
        ...
        <div class="card mb-4">
          <div class="card-header fw-bold bg-primary text-white">
            <i class="fas fa-stamp me-2"></i>Minting Settings
          </div>
          <div class="card-body">
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" name="auto_mint" value="1" id="auto_mint"
                         {{ (old('auto_mint', $settings['auto_mint'] ?? false) ? 'checked' : '') }}>
            ...
            <div class="row mb-3">
              <label class="col-sm-3 col-form-label">Auto-mint levels</label>
              <div class="col-sm-9">
                @php
        ...
        <div class="card mb-4">
          <div class="card-header fw-bold bg-primary text-white">
            <i class="fas fa-tags me-2"></i>Metadata Defaults
          </div>
          <div class="card-body">
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <input type="text" class="form-control" id="default_publisher" name="default_publisher"
                       value="{{ old('default_publisher', $settings['publisher'] ?? '') }}"
              ...
              <div class="col-sm-9">
                ...
                <select class="form-select" id="default_resource_type" name="default_resource_type">
                  @foreach($resourceTypes as $value => $label)
                    <option value="{{ $value }}" {{ (old('default_resource_type', $settings['resource_type'] ?? 'Dataset') === $value) ? 'selected' : '' }}>
                      {{ $label }}
                    </option>
                  @endforeach
                </select>
            ...
            <div class="row mb-3">
              <label for="suffix_pattern" class="col-sm-3 col-form-label">DOI Suffix Pattern</label>
              <div class="col-sm-9">
                <input type="text" class="form-control" id="suffix_pattern" name="suffix_pattern"
                       value="{{ old('suffix_pattern', $settings['suffix_pattern'] ?? '{year}/{entity_id}') }}">
                <div class="form-text">
                  Available placeholders: <code>{year}</code>, <code>{month}</code>, <code>{entity_id}</code>, <code>{slug}</code>, <code>{identifier}</code>, <code>{random}</code>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="card mb-4">
          ...
          <div class="card-body">
            ...
            <p class="small text-muted">
              Use the test API URL while developing. Test DOIs are not resolvable
              but allow you to verify your integration.
            </p>

            <h6 class="mt-3">DOI Suffix Pattern</h6>
            <p class="small text-muted">
              The suffix pattern controls how DOIs are constructed. The prefix
              is appended automatically. For example, with prefix <code>10.12345</code>
              and pattern <code>{year}/{entity_id}</code>, a DOI might be
              <code>10.12345/2026/abc123</code>.
            </p>
          </div>
        </div>
