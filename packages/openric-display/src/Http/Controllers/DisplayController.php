...
namespace OpenRiC\Display\Http\Controllers;
...
use OpenRiC\Display\Services\DisplayTypeDetector;

/**
 * Display controller.
 *
 * Adapted from Heratio AhgDisplay\Controllers\DisplayController (1385 lines).
 * All 30 action and helper methods preserved with full logic.
 * Uses PostgreSQL ILIKE instead of MySQL LIKE.
 * Uses PostgreSQL-native full-text search (to_tsvector/to_tsquery) instead of MySQL MATCH AGAINST.
 * Uses PostgreSQL table-existence check instead of MySQL SHOW INDEX.
 */
class DisplayController extends Controller
{
    protected DisplayServiceInterface $service;
    ...
    public function __construct(DisplayServiceInterface $service)
    {
    ...
    public function index()
    {
        ...
        return view('openric-display::admin', compact(
    ...
    public function browse(Request $request)
    {
        ...
        // Fuzzy search correction - placeholder for future integration
        $didYouMean = null;
        $correctedQuery = null;
        $originalQuery = $this->queryFilter;
        $esAssistedSearch = false;

        // Semantic expansion - placeholder for future integration
        ...
        // Discovery integration - placeholder for future integration
        $discoveryMode = false;
        ...
        $query = DB::table('information_object as io')
            ...
            ->leftJoin('term_i18n as level_term', function ($j) use ($culture) {
                $j->on('io.level_of_description_id', '=', 'level_term.id')->where('level_term.culture', '=', $culture);
            })
            ->leftJoin('display_object_config as doc', 'io.id', '=', 'doc.object_id')
        ...
        // Sort — PostgreSQL ILIKE for relevance
        ...
        switch ($sort) {
            ...
            case 'relevance':
                if ($this->queryFilter) {
                    $query->orderByRaw("CASE WHEN i18n.title ILIKE ? THEN 0 WHEN i18n.title ILIKE ? THEN 1 ELSE 2 END ASC", [
                        $this->queryFilter,
                        '%' . $this->queryFilter . '%',
                    ]);
                }
            ...
            case 'startdate':
                ...
                $query->groupBy('io.id', 'io.identifier', 'io.parent_id', 'i18n.title', 'i18n.scope_and_content', 'level_term.name', 'doc.object_type', 'slug.slug');
                break;
            case 'enddate':
                $query->leftJoin('event as evt_sort', 'io.id', '=', 'evt_sort.object_id');
        ...
        foreach ($objects as &$obj) {
            ...
            if (!$obj->thumbnail) {
                try {
                    ...
                    // table does not exist - library plugin not installed
                }
            }
        }
        unset($obj);
        ...
        return view('openric-display::index', compact(
    ...
    public function browseAjax(Request $request)
    {
        ...
        return view('openric-display::index', $viewData);
    }
    ...
    public function printView(Request $request)
    {
        ...
        $query = DB::table('information_object as io')
            ...
            ->leftJoin('slug', 'io.id', '=', 'slug.object_id')
            ->where('io.id', '>', 1)
            ->select('io.id', 'io.identifier', 'i18n.title', 'i18n.scope_and_content', 'level_term.name as level_name', 'doc.object_type', 'slug.slug');
        ...
        return view('openric-display::print', compact(
    ...
    public function exportCsv(Request $request)
    {
        ...
        $filename = 'openric_export_' . date('Y-m-d_His') . '.csv';
    ...
    public function changeType(Request $request)
    {
        ...
        DB::table('display_object_config')->updateOrInsert(
            ['object_id' => $objectId],
            ['object_type' => $newType, 'updated_at' => now()]
        );
    ...
    public function profiles()
    {
        ...
        return view('openric-display::admin', compact('profiles'));
    }
    ...
    public function levels(Request $request)
    {
        ...
        return view('openric-display::admin', compact('levels', 'currentDomain', 'domains'));
    }
    ...
    public function fields()
    {
        ...
        return view('openric-display::admin', compact('fields', 'fieldGroups'));
    }
    ...
    public function bulkSetType(Request $request)
    {
        ...
        return view('openric-display::admin', compact('collections', 'collectionTypes'));
    }
    ...
    public function browseSettings(Request $request)
    {
        ...
        $userId = auth()->id();
        $settings = $this->service->getBrowseSettings($userId);

        if ($request->isMethod('post')) {
            $data = [
                'use_glam_browse' => $request->input('use_glam_browse') ? true : false,
                ...
                'show_facets' => $request->input('show_facets') ? true : false,
                'remember_filters' => $request->input('remember_filters') ? true : false,
            ];

            if ($this->service->saveBrowseSettings($userId, $data)) {
                session()->flash('success', 'Browse settings saved');
            } else {
                session()->flash('error', 'Failed to save settings');
            }

            return redirect(route('display.get.settings'));
        }

        return view('openric-display::admin', compact('settings'));
    }
    ...
    public function toggleGlamBrowse(Request $request)
    {
        ...
        $success = $this->service->setGlamBrowse(auth()->id(), $enabled);
    ...
    public function saveBrowseSettingsAjax(Request $request)
    {
        if (!auth()->check()) {
            return response()->json(['success' => false, 'error' => 'Not authenticated']);
        }
        ...
        $success = $this->service->saveBrowseSettings(auth()->id(), $data);
    ...
    public function getBrowseSettings(Request $request)
    {
        ...
        $settings = $this->service->getBrowseSettings(auth()->id());
    ...
    public function resetBrowseSettings(Request $request)
    {
        ...
        $success = $this->service->resetBrowseSettings(auth()->id());

        return response()->json(['success' => $success]);
    }

    // =========================================================================
    // 18. SHOW - Single object display
    // =========================================================================

    public function show(Request $request, int $id)
    {
        $display = $this->service->getObjectDisplay($id);
        ...
        $events = DB::table('event as e')
            ...
            ->leftJoin('term_i18n as eti', function ($j) use ($culture) {
                $j->on('e.type_id', '=', 'eti.id')->where('eti.culture', '=', $culture);
            })
            ->where('e.object_id', $id)
            ->select('e.*', 'ei.name as event_name', 'ei.description as event_description',
                'ai.authorized_form_of_name as actor_name', 'eti.name as event_type')
            ->get()->toArray();
        ...
        return view('openric-display::show', compact(
            'object', 'type', 'profile', 'fields', 'events', 'subjects', 'places',
            'digitalObjects', 'children', 'breadcrumb', 'slug', 'id'
        ));
    }

    // =========================================================================
    // 19. BROWSE EMBEDDED - Embedded browse for iframes/panels
    // =========================================================================
    ...
    // =========================================================================
    // 20. REINDEX - Trigger reindex of display cache
    // =========================================================================
    ...
    public function reindex(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            foreach ($objectIds as $objectId) {
                DisplayTypeDetector::detectAndSave($objectId, true);
                $count++;
            }

            session()->flash('success', "Re-indexed $count objects");
            return redirect(route('display.index'));
        }
        ...
        return view('openric-display::admin', compact('stats'));
    }

    // =========================================================================
    // 21. GLAM SEARCH - Dedicated search page
    // =========================================================================

    public function glamSearch(Request $request)
    {
        $culture = app()->getLocale();
        ...
        if ($query !== '') {
            ...
            $rows = DB::table('information_object as io')
                ...
                ->leftJoin('slug', 'io.id', '=', 'slug.object_id')
                ->where('io.id', '>', 1)
                ->where(function ($qb) use ($likePattern) {
                    $qb->where('i18n.title', 'ILIKE', $likePattern)
                        ->orWhere('i18n.scope_and_content', 'ILIKE', $likePattern)
                        ->orWhere('io.identifier', 'ILIKE', $likePattern);
                })
                ->select('io.id', 'io.identifier', 'i18n.title', 'i18n.scope_and_content', 'slug.slug')
                ->limit(100)
                ->get();
        }

        return view('openric-display::index', ['rows' => $rows, 'searchQuery' => $query]);
    }

    // =========================================================================
    // 22. TREEVIEW PAGE - Hierarchical treeview display
    // =========================================================================

    public function treeviewPage(Request $request)
    {
        $culture = app()->getLocale();

        // Get top-level items for the tree
        ...
        return view('openric-display::index', compact('topLevel'));
    }
     ...
     * Uses PostgreSQL ILIKE instead of MySQL LIKE for case-insensitive matching.
     */
        ...
        if ($this->mediaFilter) {
            $query->whereExists(function ($q) {
                $q->select(DB::raw(1))
                    ...
                    ->whereRaw('digital_object.object_id = io.id')
                    ->whereNull('digital_object.parent_id')
                    ->whereRaw('digital_object.mime_type ILIKE ?', [$this->mediaFilter . '/%']);
            });
        }
        ...
        if ($this->titleFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ...
                    ->where('ioi.title', 'ILIKE', '%' . $this->titleFilter . '%');
            });
        }

        if ($this->identifierFilter) {
            $query->where('io.identifier', 'ILIKE', '%' . $this->identifierFilter . '%');
        }
        ...
        if ($this->scopeAndContentFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ->from('information_object_i18n as ioi')
                    ->whereRaw('ioi.id = io.id')
                    ->where('ioi.scope_and_content', 'ILIKE', '%' . $this->scopeAndContentFilter . '%');
            });
        }
        ...
        if ($this->creatorSearchFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ...
                    ->where('ai.authorized_form_of_name', 'ILIKE', '%' . $this->creatorSearchFilter . '%');
            });
        }
        ...
        if ($this->subjectSearchFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ...
                    ->where('ti.name', 'ILIKE', '%' . $this->subjectSearchFilter . '%');
            });
        }
        ...
        if ($this->placeSearchFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ...
                    ->where('ti.name', 'ILIKE', '%' . $this->placeSearchFilter . '%');
            });
        }
        ...
        if ($this->genreSearchFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ...
                    ->where('ti.name', 'ILIKE', '%' . $this->genreSearchFilter . '%');
            });
        }

        if ($this->referenceCodeFilter) {
            $query->where('io.identifier', 'ILIKE', '%' . $this->referenceCodeFilter . '%');
        }
        ...
        if ($this->extentAndMediumFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ->from('information_object_i18n as ioi')
                    ->whereRaw('ioi.id = io.id')
                    ->where('ioi.extent_and_medium', 'ILIKE', '%' . $this->extentAndMediumFilter . '%');
            });
        }
        ...
        if ($this->archivalHistoryFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ->from('information_object_i18n as ioi')
                    ->whereRaw('ioi.id = io.id')
                    ->where('ioi.archival_history', 'ILIKE', '%' . $this->archivalHistoryFilter . '%');
            });
        }
        ...
        if ($this->acquisitionFilter) {
            $query->whereExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ->from('information_object_i18n as ioi')
                    ->whereRaw('ioi.id = io.id')
                    ->where('ioi.acquisition', 'ILIKE', '%' . $this->acquisitionFilter . '%');
            });
        }
    ...
    /**
     * Check if PostgreSQL full-text search indexes are available on information_object_i18n.
    ...
    protected function isFulltextAvailable(): bool
    {
        ...
        try {
            ...
            $result = DB::select("
                ...
                WHERE tablename = 'information_object_i18n'
    ...
    /**
     * Apply text search filter with PostgreSQL full-text search (if available) or ILIKE fallback.
     * Supports both single-term and semantic (multi-term OR) search.
     *
     * Uses PostgreSQL to_tsvector/to_tsquery instead of MySQL MATCH AGAINST.
     *
                            ...
                            $inner->whereExists(function ($sub) use ($term, $q) {
                                $sub->select(DB::raw(1))
                                    ...
                                    ->where(function ($w) use ($term, $q) {
                                        $w->whereRaw("to_tsvector('simple', COALESCE(ioi.title, '')) @@ plainto_tsquery('simple', ?)", [$term])
                                            ->orWhereRaw("to_tsvector('simple', COALESCE(ioi.scope_and_content, '')) @@ plainto_tsquery('simple', ?)", [$term])
                                            ->orWhere('ioi.title', 'ILIKE', $q)
                                            ->orWhere('ioi.scope_and_content', 'ILIKE', $q);
                                    });
                            })->orWhere('io.identifier', 'ILIKE', $q);
                        } else {
                    ...
                    $qb->whereExists(function ($sub) use ($searchTerms, $q) {
                        $sub->select(DB::raw(1))
                            ...
                            ->where(function ($w) use ($searchTerms, $q) {
                                $w->whereRaw("to_tsvector('simple', COALESCE(ioi.title, '')) @@ plainto_tsquery('simple', ?)", [$searchTerms])
                                    ->orWhereRaw("to_tsvector('simple', COALESCE(ioi.scope_and_content, '')) @@ plainto_tsquery('simple', ?)", [$searchTerms])
                                    ->orWhere('ioi.title', 'ILIKE', $q)
                                    ->orWhere('ioi.scope_and_content', 'ILIKE', $q);
                            });
                    })->orWhere('io.identifier', 'ILIKE', $q);
     ...
     * Cached per request. Uses PostgreSQL information_schema for table existence check.
     */
            ...
            $qb->orWhereExists(function ($sub) use ($likePattern) {
                $sub->select(DB::raw(1))
                    ...
                    ->where(function ($w) use ($likePattern) {
                        $w->where('dim.creator', 'ILIKE', $likePattern)
                            ->orWhere('dim.headline', 'ILIKE', $likePattern)
                            ->orWhere('dim.caption', 'ILIKE', $likePattern)
                            ->orWhere('dim.keywords', 'ILIKE', $likePattern);
                    });
            });
        }
        ...
        if (in_array('museum_metadata', $sectorTables)) {
            $qb->orWhereExists(function ($sub) use ($likePattern) {
                $sub->select(DB::raw(1))
                    ...
                    ->where(function ($w) use ($likePattern) {
                        $w->where('mm.creator_identity', 'ILIKE', $likePattern)
                            ->orWhere('mm.materials', 'ILIKE', $likePattern)
                            ->orWhere('mm.techniques', 'ILIKE', $likePattern)
                            ->orWhere('mm.classification', 'ILIKE', $likePattern)
                            ->orWhere('mm.inscription', 'ILIKE', $likePattern);
                    });
            });
        }
        ...
        if (in_array('gallery_artist', $sectorTables)) {
            $qb->orWhereExists(function ($sub) use ($likePattern) {
                $sub->select(DB::raw(1))
                    ...
                    ->where(function ($w) use ($likePattern) {
                        $w->where('ga.display_name', 'ILIKE', $likePattern)
                            ->orWhere('ga.medium_specialty', 'ILIKE', $likePattern)
                            ->orWhere('ga.movement_style', 'ILIKE', $likePattern);
                    });
            });
        }
    }
