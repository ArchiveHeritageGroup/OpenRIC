{{--
  ...
  Adapted from Heratio ahg-display search.blade.php
            ...
            <div class="facets-sidebar sticky-top" style="top: 20px;">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-3">
                            <a href="{{ route('display.search') }}" class="btn btn-outline-secondary btn-sm w-100">
            ...
            @if(!empty($results['hits']))
                ...
                <nav class="mt-4">
                    <ul class="pagination justify-content-center">
                        @if($currentPage > 1)
                        <li class="page-item"><a class="page-link" href="?{{ http_build_query(array_merge($requestParams, ['from' => ($currentPage - 2) * $size])) }}"><i class="fas fa-chevron-left"></i></a></li>
                        @endif
                        @for($i = max(1, $currentPage - 2); $i <= min($pages, $currentPage + 2); $i++)
                        <li class="page-item {{ $i === $currentPage ? 'active' : '' }}"><a class="page-link" href="?{{ http_build_query(array_merge($requestParams, ['from' => ($i - 1) * $size])) }}">{{ $i }}</a></li>
                        @endfor
                        @if($currentPage < $pages)
                        <li class="page-item"><a class="page-link" href="?{{ http_build_query(array_merge($requestParams, ['from' => $currentPage * $size])) }}"><i class="fas fa-chevron-right"></i></a></li>
                        @endif
                    </ul>
                </nav>
                @endif
            @else
                <div class="alert alert-info"><i class="fas fa-info-circle me-2"></i>No results found. Try adjusting your filters or search terms.</div>
            @endif
        </div>
    </div>
</div>
