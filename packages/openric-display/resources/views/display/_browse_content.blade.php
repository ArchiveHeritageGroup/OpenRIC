{{--
  ...
  Adapted from Heratio ahg-display::display._browse_content (655 lines)
  Shared content partial for browse (used by both 2-col and full-width layouts)
--}}
@php
  // Re-establish helpers if not already set
...
<div class="multiline-header d-flex align-items-center mb-3">
  ...
  <button type="button" class="btn btn-outline-secondary ms-3" id="openSemanticSearchBtn" data-bs-toggle="modal" data-bs-target="#semanticSearchModal">
  ...
  <nav aria-label="Hierarchy breadcrumb" class="mb-2">
    <ol class="breadcrumb mb-0 small">
      <li class="breadcrumb-item">
        <a href="{{ route('display.browse') }}"><i class="fas fa-home"></i></a>
      </li>
      @foreach($breadcrumb as $crumb)
        @if(!$loop->last)
          <li class="breadcrumb-item">
            <a href="{{ displayBrowseUrl($fp, ['parent' => is_array($crumb) ? ($crumb['id'] ?? '') : ($crumb->id ?? '')]) }}">{{ is_array($crumb) ? ($crumb['title'] ?? '[Untitled]') : ($crumb->title ?? '[Untitled]') }}</a>
          </li>
        @else
...
@if($hasActiveFilters)
  <div class="d-flex flex-wrap align-items-center gap-2 mb-3 p-2 bg-light border rounded">
    ...
    @if(!empty($queryFilter))
      <a href="{{ displayBrowseUrl($fp, [], ['query', 'semantic']) }}" class="badge bg-dark text-decoration-none">
    ...
    @if(!empty($typeFilter))
      ...
      <a href="{{ displayBrowseUrl($fp, [], ['type']) }}" class="badge bg-{{ $tcActive['color'] }} text-decoration-none">
        <i class="fas {{ $tcActive['icon'] }} me-1"></i> {{ $tcActive['label'] }} <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($creatorFilter))
      <a href="{{ displayBrowseUrl($fp, [], ['creator']) }}" class="badge bg-info text-decoration-none">
        <i class="fas fa-user me-1"></i> Creator <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($subjectFilter))
      <a href="{{ displayBrowseUrl($fp, [], ['subject']) }}" class="badge bg-primary text-decoration-none">
        <i class="fas fa-tag me-1"></i> Subject <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($placeFilter))
      <a href="{{ displayBrowseUrl($fp, [], ['place']) }}" class="badge bg-secondary text-decoration-none">
        <i class="fas fa-map-marker-alt me-1"></i> Place <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($genreFilter))
      <a href="{{ displayBrowseUrl($fp, [], ['genre']) }}" class="badge bg-warning text-dark text-decoration-none">
        <i class="fas fa-theater-masks me-1"></i> Genre <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($levelFilter))
      <a href="{{ displayBrowseUrl($fp, [], ['level']) }}" class="badge bg-secondary text-decoration-none">
        <i class="fas fa-sitemap me-1"></i> Level <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($mediaFilter))
      <a href="{{ displayBrowseUrl($fp, [], ['media']) }}" class="badge bg-danger text-decoration-none">
        <i class="fas fa-photo-video me-1"></i> Media <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($repoFilter))
      <a href="{{ displayBrowseUrl($fp, [], ['repo']) }}" class="badge bg-success text-decoration-none">
        <i class="fas fa-building me-1"></i> Repository <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($hasDigital))
      <a href="{{ displayBrowseUrl($fp, [], ['hasDigital']) }}" class="badge text-decoration-none" style="background-color:#6f42c1;">
        <i class="fas fa-image me-1"></i> Digital objects <i class="fas fa-times ms-1"></i>
      </a>
    @endif

    @if(!empty($parentId))
      <a href="{{ displayBrowseUrl($fp, [], ['parent', 'topLevelOnly']) }}" class="badge bg-dark text-decoration-none">
    ...
    <a href="{{ route('display.browse') }}" class="badge bg-outline-secondary border text-dark text-decoration-none ms-1" title="Clear all filters">
...
@if(!empty($correctedQuery))
  <div class="alert alert-info alert-dismissible fade show mb-2" role="alert">
    ...
    @if(!empty($originalQuery))
      <a href="{{ displayBrowseUrl($fp, ['query' => $originalQuery, 'noCorrect' => 1]) }}" class="alert-link">
...
@if(!empty($didYouMean))
  <div class="alert alert-warning alert-dismissible fade show mb-2" role="alert">
    ...
    Did you mean:
    <a href="{{ displayBrowseUrl($fp, ['query' => $didYouMean]) }}" class="alert-link fw-bold">
      "{{ e($didYouMean) }}"
    </a>?
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  </div>
...
@if(!empty($esAssistedSearch))
  <div class="alert alert-secondary alert-dismissible fade show mb-2" role="alert">
    ...
    Results enhanced by full-text search.
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  </div>
@endif
...
<div class="d-flex flex-wrap align-items-center gap-2 mb-3">
  ...
  {{-- Print --}}
  <a href="{{ route('display.print', array_filter($fp)) }}" target="_blank" class="btn btn-outline-success btn-sm">
    <i class="fas fa-print"></i> Print
  </a>

  {{-- CSV Export --}}
  <a href="{{ route('display.export.csv', array_filter($fp)) }}" class="btn btn-outline-success btn-sm">
    <i class="fas fa-download"></i> CSV
  </a>

  {{-- View mode buttons --}}
  <a href="{{ displayBrowseUrl($fp, ['view' => 'card']) }}"
     class="btn btn-sm {{ ($viewMode ?? 'card') === 'card' ? 'btn-secondary' : 'btn-outline-secondary' }}" title="Card view" aria-label="Card view">
    <i class="fas fa-th-large"></i>
  </a>
  <a href="{{ displayBrowseUrl($fp, ['view' => 'grid']) }}"
     class="btn btn-sm {{ ($viewMode ?? 'card') === 'grid' ? 'btn-secondary' : 'btn-outline-secondary' }}" title="Grid view" aria-label="Grid view">
    <i class="fas fa-th"></i>
  </a>
  <a href="{{ displayBrowseUrl($fp, ['view' => 'table']) }}"
     class="btn btn-sm {{ ($viewMode ?? 'card') === 'table' ? 'btn-secondary' : 'btn-outline-secondary' }}" title="Table view" aria-label="Table view">
    <i class="fas fa-list"></i>
  </a>
  <a href="{{ displayBrowseUrl($fp, ['view' => 'full']) }}"
     class="btn btn-sm {{ ($viewMode ?? 'card') === 'full' ? 'btn-secondary' : 'btn-outline-secondary' }}" title="Full width" aria-label="Full width">
    <i class="fas fa-bars"></i>
  </a>

  {{-- Limit dropdown --}}
  <div class="dropdown">
    <button class="btn btn-outline-success btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown">{{ $limit ?? 30 }}/page</button>
    <ul class="dropdown-menu">
      @foreach([10, 30, 50, 100] as $lim)
        <li><a class="dropdown-item {{ ($limit ?? 30) == $lim ? 'active' : '' }}" href="{{ displayBrowseUrl($fp, ['limit' => $lim]) }}">{{ $lim }}</a></li>
      @endforeach
    </ul>
  </div>
  ...
  <div class="dropdown ms-auto">
    <button class="btn btn-outline-success btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown">Sort: {{ $sortLabels[$sort ?? 'title'] ?? 'Title' }}</button>
    <ul class="dropdown-menu dropdown-menu-end">
      @foreach($sortLabels as $sKey => $sLabel)
        <li>
          <a class="dropdown-item {{ ($sort ?? 'title') === $sKey ? 'active' : '' }}"
             href="{{ displayBrowseUrl($fp, ['sort' => $sKey]) }}">
            {{ $sLabel }}
          </a>
        </li>
      @endforeach
  ...
  <div class="dropdown">
    <button class="btn btn-outline-success btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown">{{ ($sortDir ?? 'asc') === 'asc' ? 'Asc' : 'Desc' }}</button>
    <ul class="dropdown-menu dropdown-menu-end">
      <li>
        <a class="dropdown-item {{ ($sortDir ?? 'asc') === 'asc' ? 'active' : '' }}"
           href="{{ displayBrowseUrl($fp, ['sortDir' => 'asc', 'dir' => 'asc']) }}">
          <i class="fas fa-sort-amount-up-alt me-1"></i> Ascending
        </a>
      </li>
      <li>
        <a class="dropdown-item {{ ($sortDir ?? 'asc') === 'desc' ? 'active' : '' }}"
           href="{{ displayBrowseUrl($fp, ['sortDir' => 'desc', 'dir' => 'desc']) }}">
...
@if(($digitalObjectCount ?? 0) > 0)
  <div class="d-flex align-items-center gap-2 mb-2">
    ...
    @if(empty($hasDigital))
      <a href="{{ displayBrowseUrl($fp, ['hasDigital' => 1]) }}" class="btn btn-outline-success btn-sm">
        <i class="fas fa-image"></i> With digital objects
      </a>
    @else
      <a href="{{ displayBrowseUrl($fp, [], ['hasDigital']) }}" class="btn btn-outline-success btn-sm">
...
@if(($total ?? 0) > 0)
  ...
  @if(($viewMode ?? 'card') === 'table')
    <div class="table-responsive mb-3">
      <table class="table table-bordered table-striped table-hover mb-0 browse-table" id="glam-browse-table">
        <thead>
          <tr>
            <th class="col-thumb"><span class="d-none d-md-inline">Image</span><div class="resize-handle"></div></th>
            <th>Title<div class="resize-handle"></div></th>
            <th class="col-identifier">Identifier<div class="resize-handle"></div></th>
            <th class="col-level">Level<div class="resize-handle"></div></th>
            <th>Type<div class="resize-handle"></div></th>
            <th class="col-actions">Actions<div class="resize-handle"></div></th>
          </tr>
        </thead>
        <tbody>
          @foreach($objects as $obj)
            @php
              $objType = $obj->object_type ?? $obj->type ?? 'archive';
              $otc = $typeConfig[$objType] ?? ['icon' => 'fa-question', 'color' => 'secondary', 'label' => ucfirst($objType)];
              $objTitle = $obj->title ?? '[Untitled]';
              $objSlug = $obj->slug ?? '';
              $objThumb = $obj->thumbnail ?? null;
              $objUrl = '/' . $objSlug;
            @endphp
            <tr>
              <td class="col-thumb text-center">
                @if($objThumb)
                  <a href="{{ $objUrl }}"><img src="{{ $objThumb }}" alt="" class="browse-thumb-lg" loading="lazy"></a>
                @else
                  <div class="browse-placeholder-lg mx-auto"><i class="fas {{ $otc['icon'] }}"></i></div>
                @endif
              </td>
              <td>
                <a href="{{ $objUrl }}" class="text-decoration-none fw-semibold">{{ e($objTitle) }}</a>
                @if(!empty($obj->scope_and_content))
                  <br><small class="text-muted">{{ Str::limit(strip_tags($obj->scope_and_content), 120) }}</small>
                @endif
              </td>
              <td class="col-identifier"><small>{{ e($obj->identifier ?? '') }}</small></td>
              <td class="col-level">
                @if($obj->level_name ?? false)
                  <span class="badge bg-light text-dark border">{{ e($obj->level_name) }}</span>
                @endif
              </td>
              <td><span class="badge bg-{{ $otc['color'] }}"><i class="fas {{ $otc['icon'] }} me-1"></i> {{ $otc['label'] }}</span></td>
              <td class="col-actions text-center">
                <a href="{{ $objUrl }}" class="btn btn-sm btn-outline-secondary" title="View"><i class="fas fa-eye"></i></a>
              </td>
            </tr>
            @if(!empty($discoveryMode) && !empty($obj->_discovery))
              <tr class="table-light">
                <td colspan="6" class="py-1 px-3">
                  @include('openric-display::display._discovery_meta', ['discovery' => $obj->_discovery])
                </td>
              </tr>
            @endif
          @endforeach
  ...
  @elseif(($viewMode ?? 'card') === 'grid')
    <div class="row row-cols-2 row-cols-md-4 row-cols-lg-6 g-3">
      @foreach($objects as $obj)
        @php
          $objType = $obj->object_type ?? 'archive';
          $otc = $typeConfig[$objType] ?? ['icon' => 'fa-question', 'color' => 'secondary', 'label' => ucfirst($objType)];
          $objTitle = $obj->title ?? '[Untitled]';
          $objSlug = $obj->slug ?? '';
          $objThumb = $obj->thumbnail ?? null;
          ...
          <div class="card-body">
            <div class="d-flex flex-column flex-md-row gap-3">
              <div class="full-img-bg flex-shrink-0">
                @if($objThumb)
                  <a href="{{ $objUrl }}"><img src="{{ $objThumb }}" alt="{{ e($objTitle) }}" class="full-img" loading="lazy"></a>
                @else
                  <div class="text-center text-muted"><i class="fas {{ $otc['icon'] }} fa-5x"></i></div>
                @endif
              </div>
              <div class="flex-grow-1">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <div>
                    <h4 class="mb-1"><a href="{{ $objUrl }}" class="text-decoration-none">{{ e($objTitle) }}</a></h4>
                    <span class="badge bg-{{ $otc['color'] }} me-1"><i class="fas {{ $otc['icon'] }} me-1"></i> {{ $otc['label'] }}</span>
                    @if($objLevel)
                      <span class="badge bg-light text-dark border">{{ e($objLevel) }}</span>
                    @endif
                  </div>
                  <a href="{{ $objUrl }}" class="btn btn-sm btn-outline-secondary" title="View"><i class="fas fa-eye me-1"></i> View</a>
                </div>
        ...
        <div class="card mb-2 shadow-sm">
          <div class="row g-0">
            ...
            <div class="col-md-9">
              <div class="card-body py-2">
                ...
                <p class="card-text mb-1 small">
                  ...
                  @endif
                  @if($childCount > 0)
                    <span class="mx-1">&middot;</span><a href="{{ displayBrowseUrl($fp, ['parent' => $obj->id]) }}"><i class="fas fa-folder"></i> {{ $childCount }}</a>
                  @endif
                </p>
                @if($objScope)
            ...
            <div class="col-md-1 d-flex flex-column align-items-center justify-content-center border-start gap-1">
              ...
              <button class="btn btn-outline-success btn-sm clipboard" data-clipboard-slug="{{ $objSlug }}" data-clipboard-type="informationObject" aria-label="Add to clipboard"><i class="fas fa-paperclip"></i></button>
            </div>
          </div>
          @if(!empty($discoveryMode) && !empty($obj->_discovery))
            <div class="card-footer py-1 px-3">
    ...
    <nav class="mt-4">
      <ul class="pagination justify-content-center">
        <li class="page-item {{ ($page ?? 1) <= 1 ? 'disabled' : '' }}">
          <a class="page-link" href="{{ displayBrowseUrl($fp, ['page' => max(1, $page - 1)]) }}">Previous</a>
        </li>
        @for($p = max(1, $page - 3); $p <= min($totalPages, $page + 3); $p++)
          <li class="page-item {{ $p == $page ? 'active' : '' }}">
            <a class="page-link" href="{{ displayBrowseUrl($fp, ['page' => $p]) }}">{{ $p }}</a>
          </li>
        @endfor
        <li class="page-item {{ ($page ?? 1) >= ($totalPages ?? 1) ? 'disabled' : '' }}">
          <a class="page-link" href="{{ displayBrowseUrl($fp, ['page' => $page + 1]) }}">Next</a>
        </li>
      </ul>
    </nav>
  @endif
...
@else
  ...
  <div class="text-center py-5">
    ...
    <p class="text-muted">
      ...
      <a href="{{ route('display.browse') }}">clear all filters</a>
      to start over.
    </p>
    @if(!empty($queryFilter))
      <p class="text-muted">
