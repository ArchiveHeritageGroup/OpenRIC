{{--
  Advanced Search Panel -- _advanced-search.blade.php
  Adapted from Heratio ahg-display::display._advanced-search (401 lines)
  ...
  <div class="accordion-item">
    ...
    <div id="glamAdvancedSearchPanel" class="accordion-collapse collapse{{ $showAdvanced ? ' show' : '' }}">
      <div class="accordion-body">
        <form method="get" action="{{ url('/display/browse') }}" id="glam-advanced-search-form">

          {{-- Sector Quick Filter Buttons --}}
          <div class="mb-4">
            <label class="form-label fw-bold"><i class="fas fa-layer-group me-1"></i>Search in sector</label>
            <div class="d-flex flex-wrap gap-2">
              <a href="{{ url('/display/browse?showAdvanced=1') }}" class="btn btn-outline-secondary"><i class="fas fa-globe me-1"></i>All</a>
              <a href="{{ url('/display/browse?type=archive&showAdvanced=1') }}" class="btn {{ $currentType === 'archive' ? 'btn-success' : 'btn-outline-success' }}"><i class="fas fa-archive me-1"></i>Archive</a>
              <a href="{{ url('/display/browse?type=library&showAdvanced=1') }}" class="btn {{ $currentType === 'library' ? 'btn-info text-white' : 'btn-outline-info' }}"><i class="fas fa-book me-1"></i>Library</a>
              <a href="{{ url('/display/browse?type=museum&showAdvanced=1') }}" class="btn {{ $currentType === 'museum' ? 'btn-warning' : 'btn-outline-warning' }}"><i class="fas fa-landmark me-1"></i>Museum</a>
              <a href="{{ url('/display/browse?type=gallery&showAdvanced=1') }}" class="btn {{ $currentType === 'gallery' ? 'btn-danger' : 'btn-outline-danger' }}"><i class="fas fa-palette me-1"></i>Gallery</a>
              <a href="{{ url('/display/browse?type=dam&showAdvanced=1') }}" class="btn {{ $currentType === 'dam' ? 'btn-primary' : 'btn-outline-primary' }}"><i class="fas fa-images me-1"></i>Photos</a>
            </div>
          </div>
          ...
          <div class="d-flex gap-2 justify-content-end mt-4 pt-3 border-top">
            <a href="{{ url('/display/browse') }}" class="btn btn-outline-secondary"><i class="fas fa-undo me-1"></i>Reset</a>
    ...
    <div class="dropdown">
      ...
      <ul class="dropdown-menu">
        @foreach($savedSearches as $ss)
          <li><a class="dropdown-item" href="{{ url('/display/browse?' . ($ss->query_string ?? '')) }}"><i class="fas fa-search me-2 text-muted"></i>{{ $ss->name ?? 'Saved search' }}</a></li>
        @endforeach
      </ul>
    </div>
  @endif
  @if(request()->has('query') || request()->has('title') || request()->has('type'))
    <button type="button" class="btn btn-sm btn-outline-success py-0 px-2" data-bs-toggle="modal" data-bs-target="#saveGlamSearchModal">
...
{{-- Active filter badge for top-level --}}
@if(request('topLevel') === '1')
  <div class="d-flex flex-wrap gap-2 mb-3">
    @php $removeTop = request()->except(['topLevel']); @endphp
    <a href="{{ url('/display/browse?' . http_build_query(array_merge($removeTop, ['topLevel' => '0']))) }}" class="badge bg-primary p-2 text-decoration-none text-white">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-success" id="glam-save-search-btn">Save</button>
      </div>
    </div>
  </div>
</div>
