{{--
  Field Mappings — display/fields.blade.php
  Adapted from Heratio ahg-display display/fields.blade.php
--}}
...
<div id="main-column" role="main">
  ...
  @if(!empty($fieldGroups) && count($fieldGroups))
    @foreach($fieldGroups as $groupName => $groupFields)
      @php $icon = $groupIcons[strtolower($groupName)] ?? 'fa-th-list'; @endphp
      <div class="card mb-4">
            ...
            <tbody>
              @foreach($groupFields as $field)
                <tr>
                  ...
                  <td><small>{{ $field->rico ?? '-' }}</small></td>
                  <td><small>{{ $field->isad ?? '-' }}</small></td>
                  <td><small>{{ $field->dc ?? '-' }}</small></td>
                </tr>
              @endforeach
            </tbody>
          </table>
          ...
          <tbody>
            @foreach($fields as $field)
              <tr>
                ...
                <td><code>{{ $field->code ?? '-' }}</code></td>
                <td>
                  @php $fieldType = $field->type ?? 'text'; $badgeClass = $typeBadges[strtolower($fieldType)] ?? 'bg-secondary'; @endphp
                  <span class="badge {{ $badgeClass }}">{{ $fieldType }}</span>
                </td>
                <td><code class="small">{{ $field->source_table ?? '-' }}</code></td>
                <td><code class="small">{{ $field->source_column ?? '-' }}</code></td>
                <td class="text-center">@if(!empty($field->rico))<i class="fas fa-check text-success" title="{{ $field->rico }}"></i>@else<span class="text-muted">-</span>@endif</td>
                <td class="text-center">@if(!empty($field->isad))<i class="fas fa-check text-success" title="{{ $field->isad }}"></i>@else<span class="text-muted">-</span>@endif</td>
                <td class="text-center">@if(!empty($field->dc))<i class="fas fa-check text-success" title="{{ $field->dc }}"></i>@else<span class="text-muted">-</span>@endif</td>
              </tr>
            @endforeach
          </tbody>
        </table>
