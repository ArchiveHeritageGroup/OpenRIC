{{--
  Display Profiles — display/profiles.blade.php
  Adapted from Heratio ahg-display display/profiles.blade.php
--}}
          ...
          <div class="card h-100">
            ...
            <div class="card-body">
              <table class="table table-sm table-borderless mb-0">
                <tbody>
                  <tr><th class="text-muted" width="120">Code:</th><td><code>{{ $profile->code ?? '-' }}</code></td></tr>
                  <tr><th class="text-muted">Layout:</th><td>{{ ucfirst($profile->layout_mode ?? $profile->layout ?? 'list') }}</td></tr>
                  <tr><th class="text-muted">Thumbnail:</th><td>{{ ucfirst($profile->thumbnail_size ?? 'default') }} ({{ $profile->thumbnail_position ?? 'left' }})</td></tr>
                  <tr><th class="text-muted">Default:</th><td>@if(!empty($profile->is_default))<span class="badge bg-success">Yes</span>@else<span class="badge bg-secondary">No</span>@endif</td></tr>
                </tbody>
              </table>
