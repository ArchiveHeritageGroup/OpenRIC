...
namespace OpenRicRic\Controllers;
...
class RicController extends Controller
{
    ...
    {
        if (! $this->tablesExist()) {
            return view('openric-ric::not-configured');
        }
        ...
        return view('openric-ric::index', compact(
            'fusekiSettings',
            'syncSummary',
            'queueStatus',
            'orphanCount',
    ...
    public function syncStatus(Request $request)
    {
        ...
        return view('openric-ric::sync-status', compact('items', 'entityTypes', 'statuses', 'entityType', 'status'));
    }
    ...
    public function orphans(Request $request)
    {
        ...
        return view('openric-ric::orphans', compact('items', 'counts', 'tab'));
    }
    ...
    public function queue(Request $request)
    {
        ...
        return view('openric-ric::queue', compact('items', 'counts', 'tab'));
    }
    ...
    public function logs(Request $request)
    {
        ...
        return view('openric-ric::logs', compact(
    ...
    public function config(Request $request)
    {
        ...
        return view('openric-ric::config', compact('config'));
    }
    ...
    {
        return view('openric-ric::explorer');
    }

    /**
     * Check whether Fuseki triplestore is available and has data.
     */
    private function fusekiAvailable(): bool
    {
        $fusekiStatus = $this->checkFusekiStatusQuick();
        return $fusekiStatus['online'] ?? false;
    }

    /**
     * Query Fuseki triplestore for entity counts by type.
     */
    private function getTriplestoreCounts(): array
    {
        $config = $this->getFusekiConfig();
        $endpoint = ($config['fuseki_endpoint'] ?? config('services.ric.fuseki_endpoint', 'http://localhost:3030/ric')) . '/query';
        $username = $config['fuseki_username'] ?? config('services.ric.fuseki_username', 'admin');
...
SPARQL;
        ...
        if ($result && isset($result['results']['bindings'])) {
            foreach ($result['results']['bindings'] as $row) {
                $type = $row['type']['value'] ?? '';
                $cnt = (int)($row['cnt']['value'] ?? 0);
                
                if (str_contains($type, 'RecordSet')) {
                    $counts['recordsets'] += $cnt;
                } elseif (str_contains($type, 'RecordPart')) {
                    $counts['recordsets'] += $cnt;
                } elseif (str_contains($type, 'Person')) {
                    $counts['persons'] += $cnt;
                } elseif (str_contains($type, 'CorporateBody')) {
                    $counts['corporate_bodies'] += $cnt;
                } elseif (str_contains($type, 'Family')) {
                    $counts['families'] += $cnt;
                } elseif (str_contains($type, 'Place')) {
                    $counts['places'] += $cnt;
                } elseif (str_contains($type, 'Activity') || str_contains($type, 'Event')) {
                    $counts['activities'] += $cnt;
                } elseif (str_contains($type, 'Instantiation')) {
                    $counts['instantiations'] += $cnt;
                }
            }
        }
    ...
    /**
     * Query Fuseki triplestore for records with pagination and sorting.
     */
    private function queryRecordsFromTriplestore(Request $request): array
    {
        $config = $this->getFusekiConfig();
        $endpoint = ($config['fuseki_endpoint'] ?? config('services.ric.fuseki_endpoint', 'http://localhost:3030/ric')) . '/query';
        $username = $config['fuseki_username'] ?? config('services.ric.fuseki_username', 'admin');
        ...
        $topLod = $request->input('topLod', '0');
        $page = max(1, (int)$request->input('page', 1));
        $perPage = 25;
        $offset = ($page - 1) * $perPage;

        // Build ORDER BY clause
        $orderBy = $sort === 'identifier' ? '?identifier' : '?title';
        if ($order === 'DESC') {
            $orderBy .= ' DESC';
        }

        // Build filter for top-level only (no parent)
        $topFilter = '';
        if ($topLod === '1') {
            $topFilter = 'FILTER NOT EXISTS { ?s rico:isOrWasIncludedIn ?parent }';
        }
...
SPARQL;
        ...
        $countResult = $this->executeSparql($countQuery, $endpoint, $username, $password);
        $total = isset($countResult['results']['bindings'][0]['total']['value']) 
            ? (int)$countResult['results']['bindings'][0]['total']['value'] 
            : 0;
...
SELECT DISTINCT ?s ?title ?identifier ?type WHERE {
  ?s a rico:RecordSet .
  OPTIONAL { ?s rico:title ?title }
  OPTIONAL { ?s rico:identifier ?identifier }
  OPTIONAL { ?s a ?type FILTER(STRSTARTS(STR(?type), "https://www.ica.org/standards/RiC/ontology#")) }
  $topFilter
}
ORDER BY $orderBy
...
SPARQL;
        ...
        $result = $this->executeSparql($dataQuery, $endpoint, $username, $password);
        ...
        if ($result && isset($result['results']['bindings'])) {
            foreach ($result['results']['bindings'] as $row) {
                ...
                // Extract slug/ID from URI for URL construction
                $slugParts = parse_url($uri);
                $path = $slugParts['path'] ?? '';
                $id = basename($path);
                $typeSlug = dirname($path);
                $typeSlug = basename($typeSlug);
                
                $records[] = (object)[
                    'id' => $id,
                    'title' => $row['title']['value'] ?? 'Untitled',
                    'identifier' => $row['identifier']['value'] ?? null,
                    'level_name' => $this->extractTypeName($row['type']['value'] ?? ''),
                    'slug' => $typeSlug . '-' . $id,
    ...
    /**
     * Extract human-readable type name from RiC-O URI.
     */
    private function extractTypeName(string $uri): string
    {
        if (preg_match('/#(\w+)$/', $uri, $m)) {
            // Convert camelCase to readable
            $name = preg_replace('/([a-z])([A-Z])/', '$1 $2', $m[1]);
            return ucfirst($name);
        }
        return 'Record Set';
    }
    ...
    /**
     * Public Browse — overview page with category cards.
     */
    public function browse()
    {
        // Check if Fuseki triplestore is available
        if (! $this->fusekiAvailable()) {
            return view('openric-ric::not-configured', [
                'message' => 'The RiC-O triplestore (Fuseki) is not available. '
                    . 'Please ensure Apache Jena Fuseki is running and configured correctly.',
                'title' => 'Archival Descriptions Not Available',
            ]);
        }

        // Get counts from triplestore
        $counts = $this->getTriplestoreCounts();

        return view('openric-ric::browse', compact('counts'));
    }

    /**
     * Public Browse Records — list/grid of archival records.
     */
    public function browseRecords(Request $request)
    {
        // Check if Fuseki triplestore is available
        if (! $this->fusekiAvailable()) {
            return view('openric-ric::not-configured', [
        ...
        // Query triplestore for records
        $data = $this->queryRecordsFromTriplestore($request);
        ...
        $records = new \Illuminate\Pagination\LengthAwarePaginator(
            ...
            $data['current_page'],
            [
                'path' => url('/browse/records'),
                'query' => $request->query(),
            ]
        );

        // Get empty levels collection (would come from taxonomy in triplestore)
        $levels = collect([]);

        return view('openric-ric::browse-records', compact('records', 'levels', 'sort', 'order', 'topLod'));
    }

    /**
     * Helper: Get count for information objects.
     */
    protected function getCount(string $table, string $column, $value): int
    {
        try {
    ...
    /**
     * Helper: Get actor count by type.
     */
    protected function getActorCount(string $type): int
    {
        try {
            return DB::table('actor')
                ->join('object_table', 'actor.id', '=', 'object_table.id')
                ->where('object_table.class_name', 'Qubit' . ucfirst($type))
                ->count();
        } catch (\Exception $e) {
            return 0;
        }
     ...
     */
    protected function getActivityCount(): int
    {
        try {
            return DB::table('term_i18n')
                ->join('term', 'term_i18n.id', '=', 'term.id')
                ->join('taxonomy', 'term.taxonomy_id', '=', 'taxonomy.id')
                ->where('taxonomy.id', 41)
                ->where('term_i18n.culture', 'en')
                ->count();
        } catch (\Exception $e) {
            return 0;
    ...
    /**
     * Helper: Get digital object count.
     */
    protected function getDigitalObjectCount(): int
    {
        try {
...
SPARQL;
    ...
    public function semanticSearch()
    {
        ...
        return view('openric-ric::semantic-search', compact('searchApiUrl'));
    }
    ...
    public function shaclValidate()
    {
        ...
        $shapesPath = base_path('packages/openric-ric/tools/ric_shacl_shapes.ttl');
