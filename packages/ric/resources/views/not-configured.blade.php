...
@section('title', $title ?? 'RiC Dashboard')
...
@section('content')
  <div class="multiline-header d-flex align-items-center mb-3">
    ...
    <div class="d-flex flex-column">
      <h1 class="mb-0">{{ $title ?? 'RiC Dashboard' }}</h1>
  ...
  <div class="alert alert-warning">
    ...
    <strong>{{ $title ?? 'Tables not configured.' }}</strong>
    
    @if(isset($message))
      <p class="mb-0 mt-2">{{ $message }}</p>
    @else
      <p>The required database tables for the RiC module have not been created yet.</p>
      <p>Please run the RiC migration to set up the following tables:</p>
      <ul class="mt-2 mb-0">
