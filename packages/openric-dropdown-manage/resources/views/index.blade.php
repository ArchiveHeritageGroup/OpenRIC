  ...
  <div class="col-lg-3 col-md-4 mb-4">
    <div class="card mb-3">
      ...
      <div class="card-body">
        ...
        <button type="button" class="btn btn-outline-success w-100 mb-3" data-bs-toggle="modal" data-bs-target="#createTaxonomyModal">
    ...
    <div class="card">
      <div class="card-body text-center small text-muted">
        @php
          ...
          $mappedColumns = \Illuminate\Support\Facades\DB::table('dropdown_column_map')->count();
        @endphp
  ...
  <div class="col-lg-9 col-md-8">
                    ...
                    <tbody>
                      @foreach ($items as $item)
                        <tr class="taxonomy-row" data-taxonomy="{{ $item->taxonomy }}" data-label="{{ strtolower($item->taxonomy_label) }}">
                          ...
                          <td class="text-end">
                            <a href="{{ route('dropdown.edit', $item->taxonomy) }}" class="btn btn-sm btn-outline-secondary" title="Edit Terms">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-secondary btn-move"
                                    data-taxonomy="{{ $item->taxonomy }}"
    ...
    @if ($totalTaxonomies === 0)
      <div class="alert alert-info mt-3">
        <i class="fas fa-info-circle me-2"></i>No taxonomies found. Click "Create Taxonomy" to add your first controlled vocabulary.
      </div>
    @endif
  </div>
</div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="createTaxonomyModalLabel"><i class="fas fa-plus me-2"></i>Create Taxonomy</h5>
...
<div class="modal fade" id="renameTaxonomyModal" tabindex="-1" aria-labelledby="renameTaxonomyModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="renameTaxonomyModalLabel"><i class="fas fa-pen me-2"></i>Rename Taxonomy</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        <button type="button" class="btn btn-primary" id="renameTaxonomyBtn">Save</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="moveSectionModal" tabindex="-1" aria-labelledby="moveSectionModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="moveSectionModalLabel"><i class="fas fa-arrows-alt me-2"></i>Move to Section</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
...
(function() {
  ...
  document.querySelectorAll('.btn-delete-taxonomy').forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      if (!confirm('Delete "' + label + '" and ALL its terms? This cannot be undone.')) return;
  ...
  document.getElementById('expandAll').addEventListener('click', function() {
    document.querySelectorAll('#dropdownAccordion .accordion-collapse').forEach(function(el) {
      new bootstrap.Collapse(el, { toggle: false }).show();
    });
  });
  document.getElementById('collapseAll').addEventListener('click', function() {
    document.querySelectorAll('#dropdownAccordion .accordion-collapse.show').forEach(function(el) {
  ...
  document.getElementById('taxonomySearch').addEventListener('input', function() {
    ...
    // Auto-expand sections that have visible matches
    if (q) {
      document.querySelectorAll('.section-accordion').forEach(function(section) {
        var visibleRows = section.querySelectorAll('.taxonomy-row:not([style*="display: none"])');
