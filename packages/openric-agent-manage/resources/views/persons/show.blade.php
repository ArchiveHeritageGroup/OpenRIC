...
@section('title', $entity['properties']['rico:title'][0]['value'] ?? 'Person')
@section('sidebar') @include('theme::partials.sidebar') @endsection
@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h3 mb-0">{{ $entity['properties']['rico:title'][0]['value'] ?? 'Person' }}</h1>
    <div class="btn-group">
        <a href="{{ route('persons.edit', ['iri' => urlencode($entity['iri'])]) }}" class="btn btn-outline-primary btn-sm">Edit</a>
        <form method="POST" action="{{ route('persons.destroy', ['iri' => urlencode($entity['iri'])]) }}" class="d-inline" onsubmit="return confirm('Delete?')">@csrf @method('DELETE')<button type="submit" class="btn btn-outline-danger btn-sm">Delete</button></form>
    </div>
</div>
