...
class CorporateBodyService implements CorporateBodyServiceInterface
{
    ...
    public function browse(array $filters = [], int $limit = 25, int $offset = 0): array
    {
        ...
        $countSparql = <<<'SPARQL'
            SELECT (COUNT(?iri) AS ?count) WHERE { ?iri a rico:CorporateBody . }
            SPARQL;
    ...
    public function create(array $data, string $userId, string $reason): string
    {
        ...
        return $this->triplestore->createEntity('rico:CorporateBody', $properties, $userId, $reason);
    }
