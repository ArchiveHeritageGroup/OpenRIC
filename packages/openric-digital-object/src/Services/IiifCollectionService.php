 ...
 * Adapted from Heratio AhgIiifCollection\Services\IiifCollectionService.
...
class IiifCollectionService
{
    ...
    public function getAllCollections(?int $parentId = null, bool $publicOnly = false): array
    {
        $query = DB::table('iiif_collection as c')
            ->leftJoin('iiif_collection_i18n as i18n', function ($join): void {
        ...
        return $query->orderBy('c.sort_order')->orderBy('c.name')->get()->map(function ($c): object {
            $c->display_name = $c->i18n_name ?: $c->name;
            $c->display_description = $c->i18n_description ?: $c->description;
            $c->item_count = $this->getItemCount((int) $c->id);
            return $c;
        })->all();
    }
     ...
     */
    public function getCollection(int|string $identifier): ?object
    {
        $query = DB::table('iiif_collection as c')
            ->leftJoin('iiif_collection_i18n as i18n', function ($join): void {
                $join->on('c.id', '=', 'i18n.collection_id')
        ...
        if ($collection !== null) {
            ...
            $collection->items = $this->getCollectionItems((int) $collection->id);
            $collection->subcollections = $this->getAllCollections((int) $collection->id);
        }
    ...
    public function getCollectionItems(int $collectionId): array
    {
        return DB::table('iiif_collection_item as ci')
            ...
            ->leftJoin('information_object_i18n as i18n', function ($join): void {
    ...
    public function addItem(int $collectionId, array $data): int
    {
        ...
        return (int) DB::table('iiif_collection_item')->insertGetId([
    ...
    public function getBreadcrumbs(object $collection): array
    {
        ...
        while ($current !== null) {
            ...
            if ($current->parent_id) {
                $current = $this->getCollection((int) $current->parent_id);
            } else {
                break;
            }
        }
    ...
    /**
     * Search objects for adding to collection. PostgreSQL ILIKE.
     */
    public function searchObjects(string $query): array
    {
        return DB::table('information_object as io')
            ...
            ->where(function ($q) use ($query): void {
                $q->where('i18n.title', 'ILIKE', "%{$query}%")
                    ->orWhere('io.identifier', 'ILIKE', "%{$query}%");
            })
            ->whereNotNull('do.id')
    ...
    /**
     * AJAX autocomplete for objects. PostgreSQL ILIKE.
     */
     ...
     * Adapted from Heratio ahgIiifPlugin manifest generator.
     */
